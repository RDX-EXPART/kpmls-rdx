...
Commands:
  /autotheme  — authorized users (owner, sudo, auth)
"""
...
async def autotheme_callback(client, query):
    ...
    if query.from_user.id != user_id and query.from_user.id != (await bot.get_me()).id:
...
# ─────────────────────────────────────────
#  REGISTER HANDLERS
#  CustomFilters.authorized — owner, sudo, AND auth users সবাই access পাবে
# ─────────────────────────────────────────
...
def add_handlers():
    bot.add_handler(
        MessageHandler(
            autotheme_command,
            filters=command(BotCommands.AutoThemeCommand) & CustomFilters.authorized,
        )
    )
    bot.add_handler(
    ...
    LOGGER.info("[AutoTheme] ✅ Handlers registered.")
