...
Commands:
  /autotheme  — owner/sudo only, toggle & configure

Integration:
  In bot/__main__.py:
    from bot.modules import auto_theme
...
# Day & Night style packs — used in status/progress messages
DAY_STYLE = {
    'header_icon':  '☀️',
    'dl_icon':      '📥',
    'ul_icon':      '📤',
...
def _apply_style(is_day: bool):
    ...
    LOGGER.info(f"[AutoTheme] Switched to {'Day ☀️' if is_day else 'Night 🌙'} theme")
...
# ─────────────────────────────────────────
#  PUBLIC HELPER — use in status messages
# ─────────────────────────────────────────

def get_theme_icon(key: str) -> str:
    """Fetch an icon from the active theme. key: 'dl_icon', 'ul_icon', etc."""
    style = config_dict.get('_ACTIVE_THEME_STYLE', DAY_STYLE)
    return style.get(key, '•')


def get_theme_bar(pct: float, length: int = 12) -> str:
    """Draw a progress bar using current theme characters."""
    style = config_dict.get('_ACTIVE_THEME_STYLE', DAY_STYLE)
    fill  = style['bar_fill']
...
def add_handlers():
    bot.add_handler(
        MessageHandler(autotheme_command,
                       filters=command(BotCommands.AutoThemeCommand) & CustomFilters.sudo)
    )
    bot.add_handler(
        CallbackQueryHandler(autotheme_callback, filters=regex(r'^at '))
    )
    bot.add_handler(
        MessageHandler(autotheme_input_handler, filters=CustomFilters.sudo)
    )
