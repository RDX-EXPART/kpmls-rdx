...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        buttons.ibutton(f"{'✅️' if auto_thumb == 'Enabled' or poster_msg == 'Exists' else ''} Auto Thumbnail", f"userset {user_id} auto_thumb_menu")
        ...
        buttons.ibutton(f"{'✅️' if auto_thumb == 'Enabled' else ''} Auto Thumbnail", f"userset {user_id} auto_thumb")
        buttons.ibutton(f"{'✅️' if poster_msg == 'Exists' else ''} Choose Poster", f"userset {user_id} poster")

        text = (
            f"㊂ <b><u>Auto Thumbnail Settings for {name}</u></b>\n\n"
            f"➲ <b>Auto Thumbnail :</b> <i>{auto_thumb}</i>\n"
            f"➲ <b>Choose Poster :</b> <i>{poster_msg}</i>\n\n"
            f"➲ <b>Description :</b> <i>Enable auto thumbnail or choose your own poster image for leeched videos.</i>"
        )
