...
Fix v2:
  - Upload tasks এ Pause দেখাবে না (PyroMulti support নেই)
  - Download tasks এ Pause/Resume কাজ করবে
  - সব task এ Cancel কাজ করবে
"""
...
# ─────────────────────────────────────────
#  UPLOAD STATUS DETECTION
#  PyroMulti / Telegram uploads cannot be paused
# ─────────────────────────────────────────

_UPLOAD_STATUSES = {"Upload", "Seed", "QueueUp"}


def is_pausable(download) -> bool:
    """Return True only for download-phase tasks that support pause."""
    try:
        tstatus = str(download.status())
        if tstatus in _UPLOAD_STATUSES:
            return False
        obj = download.download()
        # Aria2 downloads have pause()
        if hasattr(obj, 'pause'):
            return True
        # qBittorrent torrents
        if hasattr(obj, 'client') and hasattr(obj.client, 'torrents_pause'):
            return True
        return False
    except Exception:
        return False
    ...
    if action == 'pause':
        ...
        if not is_pausable(dl):
            ...
            return await query.answer(
                f"⚠️ '{tstatus}' task pause করা যায় না।\nShudhu Download phase এ pause হয়।",
                show_alert=True
            )
        try:
            obj = dl.download()
    ...
    # ── RESUME ─────────────────────────────
        ...
        try:
            ...
            if hasattr(obj, 'resume'):
                ...
                return await query.answer("Resume support নেই এই task এ।", show_alert=True)
