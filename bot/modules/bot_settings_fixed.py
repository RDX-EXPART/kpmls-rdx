...
async def update_private_file(_, message, pre_message):
    ...
    if not message.media and (file_name := message.text):
        ...
        if file_name == 'accounts.zip':
            ...
            if user_id in [int(config_dict.get('OWNER_ID') or globals().get('OWNER_ID', 0) or 0), int(globals().get('DEV_ID', 0) or 0)]:
...
async def edit_bot_settings(client, query):
    ...
    # Safe owner/dev id resolving for forks where OWNER_ID is not exported globally.
    owner_id = config_dict.get('OWNER_ID') or globals().get('OWNER_ID', 0)
    dev_id = globals().get('DEV_ID', 0)
