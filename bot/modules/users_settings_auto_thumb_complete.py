...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            'lprefix': ['Leech Filename Prefix is the Front Part attacted with the Filename of the Leech Files.', 'Send Leech Filename Prefix. Documentation Here : <a href="https://t.me/leechsource">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lsuffix': ['Leech Filename Suffix is the End Part attached with the Filename of the Leech Files', 'Send Leech Filename Suffix. Documentation Here : <a href="https://t.me/leechsource">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'lremname': ['Leech Filename Remname is combination of Regex(s) used for removing or manipulating Filename of the Leech Files', 'Send Leech Filename Remname. Documentation Here : <a href="https://t.me/leechsource">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            'auto_rename': ['Template based auto rename for leeched files using placeholders like {name}, {year}, {season}, {episode}, {quality}, {extension}', 'Send Auto Rename Format.\nExample:\n<code>{name} {season}{episode} {year} {resolution} {quality} {shortlang} {lib}{extension}</code>\n\n<b>Timeout:</b> 60 sec'],
            'lcaption': ['Leech Caption is the Custom Caption on the Leech Files Uploaded by the bot', 'Send Leech Caption. You can add HTML tags. Documentation Here : <a href="https://t.me/leechsource">Click Me</a> \n<b>Timeout:</b> 60 sec'],
            ...
            'poster': ['Poster image used by Auto Thumbnail system for leeched videos.', 'Send a photo to save it as Auto Thumbnail poster.\n<b>Timeout:</b> 60 sec'],
            'auto_thumb': ['Auto Thumbnail generates or uses a poster thumbnail for leeched videos before upload.', 'Toggle Auto Thumbnail from this menu.'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        buttons.ibutton("Close", f"userset {user_id} close", "footer")
        button = buttons.build_menu(2)
    elif key == 'auto_thumb_menu':
        auto_thumb = "Enabled" if user_dict.get('auto_thumb', False) else "Disabled"
        poster_msg = "Exists" if await aiopath.exists(posterpath) else "Not Exists"

        buttons.ibutton(
            f"{'✅️' if auto_thumb == 'Enabled' else ''} Auto Thumbnail",
            f"userset {user_id} auto_thumb"
        )

        buttons.ibutton(
            f"{'✅️' if poster_msg == 'Exists' else ''} Choose Poster",
            f"userset {user_id} poster"
        )

        text = (
            f"㊂ <b><u>Auto Thumbnail Settings for {name}</u></b>\n\n"
            f"➲ <b>Auto Thumbnail :</b> <i>{auto_thumb}</i>\n"
            f"➲ <b>Choose Poster :</b> <i>{poster_msg}</i>\n\n"
            f"➲ <b>Description :</b> <i>Enable auto thumbnail or choose your own poster image.</i>"
        )
        ...
        if key == 'rcc':
            ...
            set_exist = await aiopath.exists(posterpath)
            text += f"➲ <b>Choose Poster :</b> <i>{'' if set_exist else 'Not'} Exists</i>\n\n"
        elif key == 'yt_opt':
        ...
        elif key in ['lprefix', 'lremname', 'lsuffix', 'lcaption', 'ldump', 'metadata', 'lattachment', 'auto_rename']:
        ...
        if set_exist and set_exist != 'Not Exists' and (set_exist != get_readable_file_size(config_dict['LEECH_SPLIT_SIZE']) + ' (Default)'):
            if key == 'thumb':
                ...
                buttons.ibutton("View Poster", f"userset {user_id} vposter", "header")
            elif key == 'user_tds':
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'yt_opt', 'metadata', 'lattachment', 'auto_rename'] and reply_to.text:
            ...
            elif set_arg == 'poster' and reply_to.media:
                return await set_poster(client, reply_to, msg, set_arg, True)
...
async def set_poster(client, message, pre_event, key, direct=False):
    user_id = message.from_user.id
    handler_dict[user_id] = False
    path = "Posters/"
    if not await aiopath.isdir(path):
    ...
    await sync_to_async(Image.open(photo_dir).convert("RGB").save, des_dir, "JPEG")
    await aioremove(photo_dir)
    update_user_ldata(user_id, 'poster', des_dir)
    await deleteMessage(message)
    await update_user_settings(pre_event, 'poster', 'auto_thumb_menu', msg=message, sdirect=direct)
    if DATABASE_URL:
        await DbManger().update_user_doc(user_id, 'poster', des_dir)
...
async def edit_user_settings(client, query):
    ...
    poster_path = f'Posters/{user_id}.jpg'
    rclone_path = f'wcl/{user_id}.conf'
    user_dict = user_data.get(user_id, {})
    if user_id != int(data[1]):
        await query.answer("Not Yours!", show_alert=True)
    elif data[2] in ['universal', 'mirror', 'leech', 'auto_thumb_menu']:
        await query.answer()
        ...
        buttons = ButtonMaker()
        buttons.ibutton('Cʟᴏsᴇ', f'kpsmlx {user_id} close')
        await sendMessage(message, from_user.mention, buttons.build_menu(1), poster_path)
        await update_user_settings(query, 'poster', 'auto_thumb_menu')
        ...
        handler_dict[user_id] = False
        if await aiopath.exists(poster_path):
            await query.answer()
            await aioremove(poster_path)
            update_user_ldata(user_id, 'poster', '')
            await update_user_settings(query, 'poster', 'auto_thumb_menu')
            if DATABASE_URL:
                await DbManger().update_user_doc(user_id, 'poster')
        else:
            await query.answer("Old Settings", show_alert=True)
            await update_user_settings(query, 'auto_thumb_menu')
    elif data[2] == 'thumb':
        await query.answer()
        ...
        await query.answer()
        edit_mode = len(data) == 4
        await update_user_settings(query, data[2], 'auto_thumb_menu', edit_mode)
        if not edit_mode: return
        pfunc = partial(set_poster, pre_event=query, key=data[2])
        rfunc = partial(update_user_settings, query, data[2], 'auto_thumb_menu')
        await event_handler(client, query, pfunc, rfunc, True)
        ...
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['bot_pm', 'mediainfo', 'save_mode', 'td_mode', 'auto_start', 'auto_thumb']:
        handler_dict[user_id] = False
        ...
        if data[2] == 'auto_thumb':
            await query.answer()
            update_user_ldata(user_id, 'auto_thumb', not user_dict.get('auto_thumb', False))
            await update_user_settings(query, 'auto_thumb_menu')
            if DATABASE_URL:
                await DbManger().update_user_data(user_id)
            return
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'mprefix', 'msuffix', 'mremname', 'metadata', 'lattachment', 'auto_rename']:
        handler_dict[user_id] = False
        await query.answer()
        edit_mode = len(data) == 4
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['dlprefix', 'dlsuffix', 'dlremname', 'dlcaption', 'dldump', 'dmetadata', 'dlattachment', 'dauto_rename']:
        handler_dict[user_id] = False
        await query.answer()
