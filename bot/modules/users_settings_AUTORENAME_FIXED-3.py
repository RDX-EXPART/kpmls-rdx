...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        buttons.ibutton(f"📝 Auto Rename✅", f"userset {user_id} lrename")
        ...
        elif key in ['lprefix', 'lremname', 'lsuffix', 'lcaption', 'lrename', 'ldump', 'metadata', 'lattachment']:
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix','lsuffix','lremname','lcaption','lrename','ldump','yt_opt','metadata','lattachment'] and reply_to.text:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        # Toggle Auto Rename (set/remove) without closing the settings menu
        if user_dict.get('lrename'):
            user_dict.pop('lrename', None)
            user_data[user_id] = user_dict
            await query.answer("Auto Rename OFF")
            await update_user_settings(query, 'leech')
        else:
            handler_dict[user_id] = 'lrename'
            await query.answer()
            msg = ("Send your Auto Rename template as reply.\n\n"
                   "Example:\n"
                   "{name} {season}{episode} {year} {resolution} {quality} {shortlang}{extension}\n\n"
                   "Use Back to return.")
            await update_user_settings(query, 'lrename', 'leech', True, msg)
        return
