...
Commands:
  /bandwidth  — authorized users (owner, sudo, auth)
"""
...
# ─────────────────────────────────────────
#  REGISTER HANDLERS
#  CustomFilters.authorized — owner, sudo, AND auth users
# ─────────────────────────────────────────
...
def add_handlers():
    bot.add_handler(
        MessageHandler(
            bandwidth_command,
            filters=command(BotCommands.BandwidthCommand) & CustomFilters.authorized,
        )
    )
    bot.add_handler(
    ...
    LOGGER.info("[BandwidthMgr] ✅ Handlers registered.")
