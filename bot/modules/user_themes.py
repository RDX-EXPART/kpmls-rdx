...
async def mytheme_callback(client, query):
    ...
    if action == 'section':
        ...
        if section == 'progress':
            ...
            buttons = ButtonMaker()
            for key, c in CAPTION_STYLES.items():
                label = ("✅ " if cfg['caption_style'] == key else "") + c['name']
    ...
    if action == 'set':
        ...
        buttons.ibutton("⬅️ Back", f"theme {user_id} back")
        preview = build_preview_message(cfg['progress_style'], cfg['emoji_pack'], cfg['caption_style'])
        section_titles = {'progress': '🎨 Progress Bar Style', 'emoji': '😊 Emoji Pack', 'caption': '📄 Caption Style'}
        await editMessage(
            query.message,
