...
from asyncio import create_subprocess_exec, create_subprocess_shell, sleep, gather, create_task
...
from bot import config_dict, user_data, DATABASE_URL, MAX_SPLIT_SIZE, list_drives_dict, categories_dict, aria2, GLOBAL_EXTENSION_FILTER, status_reply_dict_lock, Interval, aria2_options, aria2c_global, IS_PREMIUM_USER, download_dict, qbit_options, get_client, LOGGER, bot, extra_buttons, shorteners_list, DEV_ID, OWNER_ID
...
async def update_private_file(_, message, pre_message):
    ...
    if not message.media and (file_name := message.text):
        ...
        if file_name == 'accounts.zip':
            ...
            if user_id in [globals().get('OWNER_ID', config_dict.get('OWNER_ID', 0)), globals().get('DEV_ID', config_dict.get('DEV_ID', 0))]:
...
async def event_handler(client, query, pfunc, rfunc, document=False):
    ...
    hkey = (chat_id, user_id)
    handler_dict[hkey] = True

    async def event_filter(_, __, event):
        user = event.from_user or event.sender_chat
        if not user:
            return False
        mtype = (event.text or event.document) if document else event.text
        return bool(user.id == user_id and event.chat.id == chat_id and mtype)

    handler_ref = None

    async def wrapped_pfunc(client_, message_):
        handler_dict[hkey] = False
        try:
            await pfunc(client_, message_)
        finally:
    ...
    handler_ref = client.add_handler(MessageHandler(wrapped_pfunc, filters=create(event_filter)), group=-1)

    async def timeout_watcher():
        await sleep(300)
        if handler_dict.get(hkey):
            handler_dict[hkey] = False
            try:
                client.remove_handler(*handler_ref)
            except Exception as e:
...
async def edit_bot_settings(client, query):
    ...
    user_id = query.from_user.id
    owner_id = globals().get('OWNER_ID', config_dict.get('OWNER_ID', 0))
    dev_id = globals().get('DEV_ID', config_dict.get('DEV_ID', 0))
    if len(data) > 2:
