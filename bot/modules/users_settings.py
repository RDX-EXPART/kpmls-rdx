...
from bot.helper.ext_utils.bot_utils import getdailytasks, update_user_ldata, get_readable_file_size, sync_to_async, is_gdrive_link
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        if key == 'rcc':
            ...
            auto_thumb = user_dict.get('auto_thumbnail', False)
            choose_poster = user_dict.get('choose_poster', False)
            text += f"➲ <b>Custom Thumbnail :</b> <i>{'' if set_exist else 'Not'} Exists</i>\n"
            text += f"➲ <b>Auto Thumbnail :</b> <i>{'Enabled' if auto_thumb else 'Disabled'}</i>\n"
            text += f"➲ <b>Choose Poster :</b> <i>{'Enabled' if choose_poster else 'Disabled'}</i>\n\n"
            buttons.ibutton(f"{'✅️' if auto_thumb else ''} Auto Thumbnail", f"userset {user_id} auto_thumbnail", "header")
            buttons.ibutton(f"{'✅️' if choose_poster else ''} Choose Poster", f"userset {user_id} choose_poster", "header")
        elif key == 'yt_opt':
            ...
            if key == 'lcaption':
                _ai_cap_val = user_dict.get('ai_caption', '')
                _ai_cap_is_on = (_ai_cap_val is True) or (_ai_cap_val == '' and config_dict.get('AI_CAPTION', False))
                _ai_cap_src = '' if _ai_cap_val != '' else ' (Global)'
                buttons.ibutton(
                    f"{'✅️' if _ai_cap_is_on else ''} 🤖 AI Caption: {'ON' if _ai_cap_is_on else 'OFF'}{_ai_cap_src}",
                    f"userset {user_id} ai_caption",
                    "header"
                )
                text += f"➲ <b>🤖 AI Caption :</b> <i>{'✅ ON' if _ai_cap_is_on else '❌ OFF'}{_ai_cap_src}</i>\n\n"
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await sendMessage(message, from_user.mention, buttons.build(1), thumb_path)
        ...
        if await aiopath.exists(thumb_path):
            ...
            await update_user_settings(query, 'leech')
    elif data[2] == 'auto_thumbnail':
        handler_dict[user_id] = False
        new_mode = not user_dict.get('auto_thumbnail', False)
        update_user_ldata(user_id, 'auto_thumbnail', new_mode)
        if not new_mode and user_dict.get('choose_poster', False):
            update_user_ldata(user_id, 'choose_poster', False)
        await query.answer('Auto Thumbnail Enabled' if new_mode else 'Auto Thumbnail Disabled')
        await update_user_settings(query, 'thumb', 'leech')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] == 'choose_poster':
        handler_dict[user_id] = False
        if not user_dict.get('auto_thumbnail', False):
            return await query.answer('Enable Auto Thumbnail first!', show_alert=True)
        update_user_ldata(user_id, 'choose_poster', not user_dict.get('choose_poster', False))
        await query.answer()
        await update_user_settings(query, 'thumb', 'leech')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
        ...
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['bot_pm', 'mediainfo', 'save_mode', 'td_mode', 'auto_start']:
        handler_dict[user_id] = False
        ...
        if current == '':
            ...
            g_status = '✅ ON' if config_dict.get('AI_CAPTION') else '❌ OFF'
            await query.answer(f'🔄 Reset to Global Default ({g_status})', show_alert=False)
        await update_user_settings(query, 'lcaption', 'leech')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
        ...
        buttons.ibutton("Close", f"userset {user_id} close", "footer")
        await editMessage(message, 'Do you want to Reset Settings ?', buttons.build(2))
