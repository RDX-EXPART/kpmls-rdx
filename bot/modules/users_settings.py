...
fname_dict = {'rcc': 'RClone',
             ...
             'lsuffix': 'Suffix',
             'lremname': 'Remname',
    'lrename': 'Send Auto Rename template. Use placeholders like {name} {season}{episode} {year} {resolution} {quality} {shortlang}{extension}.',
             'metadata': 'Metadata',
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        buttons.ibutton(f"{'✅️' if (lrename:=user_dict.get('lrename','')) else ''} Auto Rename", f"userset {user_id} lrename")
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'lrename', 'lrename', 'ldump', 'yt_opt', 'metadata', 'lattachment'] and reply_to.text:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await event_handler(client, query, pfunc, rfunc)
    elif data[2] in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'lrename', 'ldump', 'mprefix', 'msuffix', 'mremname', 'metadata', 'lattachment']:
        handler_dict[user_id] = False
        await query.answer()
        edit_mode = len(data) == 4
