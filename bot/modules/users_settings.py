...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            ...
            'auto_thumbnail': ['Auto thumbnail uses GK API to fetch a poster from the movie/series/file name.', 'Toggle Auto Thumbnail ON/OFF.'],
            'choose_poster': ['Choose Poster works only when Auto Thumbnail is ON. It uses the API poster before upload.', 'Toggle Choose Poster ON/OFF.'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        choose_poster = user_dict.get('choose_poster', False) and auto_thumb
        buttons.ibutton(f"{'✅️' if auto_thumb else ''} Auto Thumbnail", f"userset {user_id} auto_thumbnail")
        buttons.ibutton(f"{'✅️' if choose_poster else ''} Choose Poster", f"userset {user_id} choose_poster")
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        handler_dict[user_id] = False
        new_state = not user_dict.get('auto_thumbnail', False)
        update_user_ldata(user_id, 'auto_thumbnail', new_state)
        if not new_state:
            update_user_ldata(user_id, 'choose_poster', False)
        await query.answer('Auto Thumbnail Enabled' if new_state else 'Auto Thumbnail Disabled')
        await update_user_settings(query, 'leech')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
        ...
        new_state = not user_dict.get('choose_poster', False)
        update_user_ldata(user_id, 'choose_poster', new_state)
        await query.answer('Choose Poster Enabled' if new_state else 'Choose Poster Disabled')
        await update_user_settings(query, 'leech')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
