...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            ...
            'choose_poster': ['Choose Poster enables automatic poster selection from supported sources.', 'Toggle Choose Poster from Auto Thumbnail menu.'],
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        choose_poster = "Enabled" if user_dict.get('choose_poster', False) else "Disabled"

        buttons.ibutton(
            f"{'✅️' if auto_thumb == 'Enabled' or choose_poster == 'Enabled' else ''} Auto Thumbnail",
        ...
        buttons.ibutton(
            f"{'✅️' if choose_poster == 'Enabled' else ''} Choose Poster",
            f"userset {user_id} choose_poster"
        )

        text = (
            f"㊂ <b><u>Auto Thumbnail Settings for {name}</u></b>\n\n"
            f"➲ <b>Auto Thumbnail :</b> <i>{auto_thumb}</i>\n"
            f"➲ <b>Choose Poster :</b> <i>{choose_poster}</i>\n\n"
            f"➲ <b>Description :</b> <i>Enable auto thumbnail and automatic poster selection.</i>"
        )
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
    elif data[2] in ['bot_pm', 'mediainfo', 'save_mode', 'td_mode', 'auto_start', 'auto_thumb', 'choose_poster']:
        handler_dict[user_id] = False
        ...
        if data[2] in ['auto_thumb', 'choose_poster']:
            await query.answer()
