...
from asyncio import create_subprocess_exec, gather, run as asyrun, create_task
...
from .modules import authorize, clone, gd_count, gd_delete, gd_list, cancel_mirror, mirror_leech, status, torrent_search, torrent_select, ytdlp, \
                     ...
                     gd_clean, broadcast, category_select, auto_start, \
                     user_themes, user_levels, night_mode

try:
    from .modules import auto_theme, bandwidth_manager, inline_task_control, session_restore
    _NEW_FEATURES_LOADED = True
    LOGGER.info("[NewFeatures] All 4 feature modules imported successfully.")
except Exception as _nf_err:
    _NEW_FEATURES_LOADED = False
    LOGGER.error(f"[NewFeatures] Failed to import feature modules: {_nf_err}")
    auto_theme = bandwidth_manager = inline_task_control = session_restore = None
...
async def main():
    ...
    # ── Existing Features ────────────────────────────────────────────
    from bot.modules.night_mode import night_mode_loop, inject_defaults
    inject_defaults()
    create_task(night_mode_loop())
    user_themes.add_handlers()
    user_levels.add_handlers()
    night_mode.add_handlers()

    # ── New Features (safe load — errors logged, bot still starts) ──
    if _NEW_FEATURES_LOADED:
        # Feature 1: Auto Dark/Light Theme
        try:
            from bot.modules.auto_theme import auto_theme_loop, inject_defaults as at_inject
            at_inject()
            create_task(auto_theme_loop())
            auto_theme.add_handlers()
            LOGGER.info("[NewFeatures] ✅ Auto Theme ready.")
        except Exception as e:
            LOGGER.error(f"[NewFeatures] ❌ Auto Theme failed: {e}")

        # Feature 2: Smart Bandwidth Manager
        try:
            ...
            LOGGER.info("[NewFeatures] ✅ Bandwidth Manager ready.")
        except Exception as e:
            LOGGER.error(f"[NewFeatures] ❌ Bandwidth Manager failed: {e}")

        # Feature 3: Inline Task Control Buttons
        try:
            inline_task_control.add_handlers()
            LOGGER.info("[NewFeatures] ✅ Inline Task Control ready.")
        except Exception as e:
            LOGGER.error(f"[NewFeatures] ❌ Inline Task Control failed: {e}")

        # Feature 4: Session Restore
        try:
            from bot.modules.session_restore import SessionRestore, inject_defaults as sr_inject
            sr_inject()
            sr = SessionRestore()
            await sr.restore_on_startup()
            session_restore.add_handlers()
            LOGGER.info("[NewFeatures] ✅ Session Restore ready.")
        except Exception as e:
            LOGGER.error(f"[NewFeatures] ❌ Session Restore failed: {e}")
    else:
        LOGGER.warning("[NewFeatures] ⚠️ Skipped — modules failed to import. Check logs above.")
    # ────────────────────────────────────────────────────────────────
