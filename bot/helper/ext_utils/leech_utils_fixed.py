...
def _rdx_parse_fields(raw_filename: str) -> dict:
    ...
    # Remove common site/source prefixes like www.1TamilMV.LTD
    text = re_sub(r'www\.[^\s]+', ' ', original, flags=IGNORECASE)
    text = re_sub(r'\b\d*TamilMV\b|\bTamilMV\b', ' ', text, flags=IGNORECASE)

    # Normalize separators and brackets for clean parsing
    ...
    # Season / Episode
    season = episode = ''
    m = re_search(r'\bS(\d{1,2})E(\d{1,3})\b', search_text, IGNORECASE)
    if m:
    ...
    # Quality
    quality = ''
    for q in ['WEB-DL', 'WEBRip', 'HDRip', 'BluRay', 'DVDRip', 'HDTV', 'CAMRip', 'CAM']:
    ...
    if lm:
        lib = lm.group(1)
        lib = lib.replace('H.264', 'x264').replace('H.265', 'x265')
    ...
    # Subtitle short
    shortsub = ''
    for s in ['ESub', 'MSub', 'HSub', 'Sub']:
        if re_search(rf'\b{s}\b', search_text, IGNORECASE):
            shortsub = s
            break
