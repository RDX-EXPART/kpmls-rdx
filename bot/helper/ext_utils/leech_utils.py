...
def _rdx_parse_fields(raw_filename: str) -> dict:
    ...
    # Quality
    quality = ''
    for q in ['WEB-DL', 'WEBRip', 'HDRip', 'HQ HDRip', 'BluRay', 'BRRip', 'DVDRip', 'HDTV', 'CAMRip', 'CAM']:
        q_pattern = re_sub('-', r'[- ]?', q)
        if re_search(rf'\b{q_pattern}\b', search_text, IGNORECASE):
            quality = 'HDRip' if q == 'HQ HDRip' else q
            break
    ...
    title = re_sub(r'\b(Hi|Hin|Eng|Tam|Tel|Mal|Kan|Ml|Kn|Te|Ta)\b', ' ', title, flags=IGNORECASE)
    title = re_sub(r'\b\d\s+\d\b', ' ', title)
    title = re_sub(r'\s*\+\s*', ' ', title)
    title = re_sub(r'\b(immo|www|frl|rls|source|original|untouched|true)\b', ' ', title, flags=IGNORECASE)
    title = re_sub(r'\b\d{1,2}\s+\d{2}\s*(?:A\s*M|P\s*M|AM|PM)\b', ' ', title, flags=IGNORECASE)
    title = re_sub(r'\b\d{1,2}\s+\d{2}\s+(?:A|P)\s+(?:M)\b', ' ', title, flags=IGNORECASE)
    title = re_sub(r'\s+', ' ', title).strip(' -._')

    title = re_sub(r'\b\d{1,3}\s+\d{1,3}\b', ' ', title)
    title = re_sub(r'\bEP\s*\d{1,3}\b', ' ', title, flags=IGNORECASE)
    title = re_sub(r'\s+', ' ', title).strip(' -._')
...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    if auto_rename and isinstance(auto_rename, str) and auto_rename.strip():
        ...
        # If files are extracted from a season pack, episode filenames may contain episode titles
        # like "Matka King Taqdeer Tadbeer Tarkeeb S01E01...".
        # In that case use the parent/extracted folder name as the clean series title.
        if dirpath and _meta.get("season") and _meta.get("episode"):
            with suppress(Exception):
                ...
                if _parent_meta.get("name") and (
                    not _parent_meta.get("season") or _parent_meta.get("season") == _meta.get("season")
                ):
                    _meta["name"] = _parent_meta["name"]
        ...
        if dirpath:
            ...
            if await aiopath.exists(up_path):
                try:
                    ...
                    if lang:
                        # Merge ffprobe stream languages with filename-detected languages.
                        # Some videos have incomplete stream tags, so filename languages must be preserved.
                        _probe_langs = [x.strip() for x in lang.split(",") if x.strip()]
                        _name_langs = [x.strip() for x in _meta.get("languages", "").replace(",", " ").split() if x.strip()]
                        _merged_langs = []
                        _seen_langs = set()
                        for _lg in _name_langs + _probe_langs:
                            _key = _lg.casefold()
                            if _key and _key not in _seen_langs:
                                _merged_langs.append(_lg)
                                _seen_langs.add(_key)
                        if _merged_langs:
                            _meta["languages"] = ' '.join(_merged_langs)
                            if len(_merged_langs) == 1:
                                _meta["shortlang"] = _merged_langs[0]
                            elif len(_merged_langs) == 2:
                                _meta["shortlang"] = "Dual"
                            elif len(_merged_langs) > 2:
                                _meta["shortlang"] = f"Multi{len(_merged_langs)}"
                    if subs:
