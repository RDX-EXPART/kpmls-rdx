...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    # ── WZML-X Auto Rename Engine ─────────────────────────────────────────────
    if auto_rename and isinstance(auto_rename, str) and auto_rename.strip():
        try:
            from bot.helper.ext_utils.wzml_rename import (
                fetch_extra_info, safe_format,
                get_video_bit_codec, sanitize_filename as wzml_sanitize,
            )
            Info = fetch_extra_info(file_)
            Extension = ospath.splitext(prefile_)[1]

            if Info.AutoRename:
                # ── ffprobe: real resolution / languages / subtitles / duration ──
                dur_  = 0; _Resolution = ""; _Languages = ""; _Subtitles = ""
                bitrate_ = ""; codec_ = ""
                if dirpath:
                    _up = ospath.join(dirpath, prefile_)
                    if await aiopath.exists(_up):
                        try:
                            dur_, _Resolution, _Languages, _Subtitles = await get_media_info(_up, True)
                            bitrate_, codec_ = await get_video_bit_codec(_up)
                        except Exception as _me:
                            LOGGER.warning(f"WZML MediaInfo: {_me}")

                Resolution  = (Info.Resolution or _Resolution)
                if bitrate_:
                    Resolution = f"{Resolution} {bitrate_}".strip()

                _probe_langs = [L.strip() for L in _Languages.split(",") if L.strip()]
                Languages    = _probe_langs if _probe_langs else (Info.Languages or [])
                _probe_subs  = [S.strip() for S in _Subtitles.split(",") if S.strip()]
                Subtitles    = _probe_subs if _probe_subs else []

                total_lang = len(Languages)
                total_sub  = len(Subtitles)
                ShortLang  = ("Single" if total_lang == 1 else
                              "Dual"   if total_lang == 2 else
                              f"Multi{total_lang}" if total_lang > 2 else "")
                ShortSub   = ("" if total_sub == 0 else
                              f"{Subtitles[0][0:1].upper()}Sub" if total_sub == 1 else "MSub")

                lib_ = codec_ or Info.Lib

                _p  = prefix.replace('\s', ' ') if prefix else ""
                _s  = suffix.replace('\s', ' ') if suffix else ""
                _ep = (f"{Info.Start_Episode}-{Info.End_Episode[1:]}"
                       if Info.End_Episode else Info.Start_Episode)

                GK_FORMAT = (
                    f"{_p}{Info.Name} {Info.Season}{_ep} {Info.Year} "
                    f"{Resolution} {Info.Ott} {Info.Quality} "
                    f"{' '.join(Languages)} {Info.Audio} {lib_} {ShortSub}{_s}{Extension}"
                )
                GK_FORMAT = re_sub(r'[^\S\n]+', ' ', GK_FORMAT.strip())

                _fsize = ""
                if dirpath:
                    _up2 = ospath.join(dirpath, prefile_)
                    if await aiopath.exists(_up2):
                        with suppress(Exception):
                            _fsize = get_readable_file_size(await aiopath.getsize(_up2))

                _tpl      = auto_rename.replace(' ', 'π')
                _new_name = safe_format(
                    _tpl,
                    gk           = GK_FORMAT,
                    file_name    = file_,
                    file_size    = _fsize,
                    ...
                    duration     = get_readable_time(dur_),
                    ott          = Info.Ott,
                ...
                _new_name = re_sub(r'[^\S\n]+', ' ', _new_name.strip()).replace('π', ' ')
                if Info.Part:
                    _n, _e = ospath.splitext(_new_name)
                    _new_name = f"{_n} {Info.Part}{_e}"
                file_ = wzml_sanitize(_new_name)
                if Extension and not file_.lower().endswith(Extension.lower()):
                    file_ = file_ + Extension

            else:
                # User set a literal rename (no placeholders or unrecognised file)
                file_ = wzml_sanitize(auto_rename.strip()) + Extension

        except Exception as _wzml_err:
            LOGGER.warning(f"WZML Auto Rename failed [{prefile_}]: {_wzml_err}")
    # ─────────────────────────────────────────────────────────────────────────
