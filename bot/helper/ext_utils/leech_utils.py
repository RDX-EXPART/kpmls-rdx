...
async def split_file(path, size, file_, dirpath, split_size, listener, start_time=0, i=1, inLoop=False, multi_streams=True):
    # Track output files so SplitStatus can show real progress instead of 0%.
    listener._split_source = path
    listener._split_dirpath = dirpath
    listener._split_filename = file_
    listener._split_start_time = getattr(listener, "_split_start_time", time())
...
_RDX_SOURCE_LEFTOVERS = [
    "center", "buzz", "tamilmv", "1tamilmv", "vegamovies", "moviesmod",
    "hdhub4u", "movieverse", "bollyflix", "filmyzilla", "mkvcinemas"
]

_RDX_KNOWN_SERIES_TITLES = (
    # Common series names where release files often add episode titles before SxxEyy.
    # Example: "Citadel Baked Alaskas S02 E01" should become "Citadel S02E01",
    # not "Citadel Baked Alaskas S02E01". Keep longer names first.
    "Never Have I Ever",
    "House of the Dragon",
    "Game of Thrones",
    "The Walking Dead",
...
def _rdx_fix_series_title(title: str) -> str:
    """Best-effort cleanup for series episode names.

    Some release names place episode titles before SxxEyy, for example:
    "Citadel Baked Alaskas S02 E01 ...". This function keeps only the
    actual show title when we can detect it safely, so Auto Rename does not
    add extra episode-title words into {name}.
    """
    if not title:
        return title
    cleaned = title.strip()
    # Remove source/domain leftovers that can remain from odd TLDs like `.center`.
    cleaned = re_sub(
        rf"^(?:{'|'.join(_RDX_SOURCE_LEFTOVERS)})\s+",
        "",
        cleaned,
        flags=IGNORECASE,
    ).strip()
    cleaned = re_sub(r"\s+", " ", cleaned).strip(" -._")

    # Prefer known series title prefixes. This prevents episode titles from
    # leaking into {name} while preserving multi-word show names like
    # "Never Have I Ever" and "The Boys".
    low = cleaned.casefold()
    for show in sorted(_RDX_KNOWN_SERIES_TITLES, key=len, reverse=True):
        show_low = show.casefold()
        if low == show_low or low.startswith(show_low + " "):
            return show

    # If a title contains a dash/colon style episode title, keep left side.
    # Example: "Show Name - Episode Title" -> "Show Name".
    sep = re_search(r"\s+(?:-|–|—|:)\s+", cleaned)
    if sep:
        left = cleaned[:sep.start()].strip(" -._")
        if 2 <= len(left) <= 60:
            return left

    # Conservative episode-title heuristic. Applies when the extra part looks
    # like a sentence/question/exclamation after a short show name.
    m = re_search(
        r"^(.{2,45}?)\s+((?:What|When|Where|Who|Why|How)\b.{3,80}[!?])$",
        cleaned,
        IGNORECASE,
    )
    if m:
        main = m.group(1).strip(" -._")
        episode = m.group(2).strip()
        if main and len(main.split()) <= 4 and len(episode.split()) >= 2:
            return main

    # Episode-title suffix trimming for series filenames with 3+ word titles.
    # Handles patterns like "Legends Could You Offer More" → "Legends"
    # and "Show Name Episode Title" → "Show Name".
    # Only triggered for series (season+episode detected) with 3+ word titles.
    # SAFE: only trims when ALL conditions are met:
    #   1. candidate is 1-2 words
    #   2. first removed word is a modal/interrogative verb (episode-title starter)
    #   3. all removed words are Title Case, not connector words, no digits, len>=2
    _connector_words = frozenset({
        'the', 'a', 'an', 'of', 'in', 'on', 'at', 'by', 'for',
        'with', 'from', 'to', 'and', 'or', 'but', 'nor', 'yet',
        'is', 'are', 'was', 'were', 'be', 'been', 'as', 'its',
        'my', 'our', 'your', 'his', 'her', 'their', 'its',
    })
    # Modal/interrogative verbs that typically start episode titles, not show names
    _episode_title_starters = frozenset({
        'could', 'would', 'should', 'can', 'will', 'did', 'does',
        'has', 'have', 'had', 'do', 'may', 'might', 'must', 'shall',
        'what', 'when', 'where', 'who', 'whom', 'whose', 'which', 'why', 'how',
    })
    words = cleaned.split()
    if len(words) >= 3:
        for trim_n in range(1, len(words)):
            candidate = ' '.join(words[:-trim_n])
            removed = words[-trim_n:]
            candidate_word_count = len(candidate.split())
            if (
                1 <= candidate_word_count <= 2
                and removed[0].lower() in _episode_title_starters
                and all(
                    w and w[0].isupper() and w.lower() not in _connector_words
                    and len(w) >= 2
                    for w in removed
                )
                and not any(re_search(r'\d', w) for w in removed)
            ):
                return candidate
...
def _rdx_parse_fields(raw_filename: str) -> dict:
    ...
    text = re_sub(
        ...
        r'[a-z0-9-]*\.(?:com|net|org|in|cc|co|xyz|lol|ltd|is|tv|to|me|info|site|live|pro|dev|foo|center|buzz)\b',
        ' ',
        text,
        flags=IGNORECASE
    )
    ...
    text = re_sub(
        r'\b(com|net|org|in|cc|co|xyz|lol|ltd|is|tv|to|me|info|site|live|pro|dev|foo|center|buzz)\b',
        ' ',
        text,
        flags=IGNORECASE
    )
    ...
    # Remove common source/site leftovers that may remain after domain cleanup.
    text = re_sub(r'\b(?:center|buzz|tamilmv|1tamilmv|vegamovies|moviesmod|hdhub4u|movieverse|bollyflix|filmyzilla|mkvcinemas)\b', ' ', text, flags=IGNORECASE)
    ...
    if season and episode:
        title = _rdx_fix_series_title(title)
    else:
        # Movie: try to clean subtitle-style extra words after the real title.
        # Safe heuristic: if title is 3+ words and the last 1-3 words look like
        # an appended subtitle (all Title Case, no digits, not connector words,
        # starts with a modal/question/article not typical of movie titles),
        # strip them so "{name}" is just the core movie title.
        _conn = frozenset({
            'the','a','an','of','in','on','at','by','for','with','from',
            'to','and','or','but','nor','yet','is','are','was','were',
            'be','been','as','its','my','our','your','his','her','their',
        })
        _ep_starters = frozenset({
            'could','would','should','can','will','did','does',
            'has','have','had','do','may','might','must','shall',
            'what','when','where','who','whom','whose','which','why','how',
        })
        _mv_words = title.split()
        if len(_mv_words) >= 3:
            for _trim in range(1, min(4, len(_mv_words))):
                _cand = ' '.join(_mv_words[:-_trim])
                _rem  = _mv_words[-_trim:]
                if (
                    1 <= len(_cand.split()) <= 3
                    and _rem[0].lower() in _ep_starters
                    and all(
                        w and w[0].isupper() and w.lower() not in _conn and len(w) >= 2
                        for w in _rem
                    )
                    and not any(re_search(r'\d', w) for w in _rem)
                ):
                    title = _cand
                    break
...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    if auto_rename and isinstance(auto_rename, str) and auto_rename.strip():
        ...
        # Traverse up to 3 directory levels to find a meaningful series name.
        # Skips generic/temp/hash-named directories automatically.
        if dirpath and _meta.get("season") and _meta.get("episode"):
            with suppress(Exception):
                _skip_dir_pattern = re_search
                _d = dirpath.rstrip("/")
                for _ in range(3):
                    _dirname = ospath.basename(_d)
                    if _dirname:
                        _pmeta = _rdx_parse_fields(_dirname)
                        pname = (_pmeta.get("name") or "").strip()
                        if (
                            pname
                            and len(pname) > 3
                            and not pname.isdigit()
                            and not re_search(
                                r'^(tmp|temp|download|downloads|splited|splited_files_mltb|\d+|[a-f0-9]{8,}|\..*|mltb.*)$',
                                pname,
                                IGNORECASE,
                            )
                        ):
                            _meta["name"] = pname
                            break
                    _next = ospath.dirname(_d)
                    if not _next or _next == _d:
                        break
                    _d = _next
        ...
        # ── IMDB lookup for {imdb_name} ──────────────────────────
        # Only fetch if the template actually uses {imdb_name}, to avoid
        # unnecessary network calls when the user hasn't asked for it.
        if '{imdb_name}' in auto_rename and not _meta.get('imdb_name'):
            try:
                from bot.helper.ext_utils.ai_caption import _fetch_imdb_data
                _imdb = await _fetch_imdb_data(_meta.get('name', ''), _meta.get('year', ''))
                if _imdb and _imdb.get('title'):
                    _meta['imdb_name'] = _imdb['title']
                    # Also backfill year from IMDB if not already present
                    if not _meta.get('year') and _imdb.get('year'):
                        _meta['year'] = _imdb['year']
                else:
                    _meta['imdb_name'] = _meta.get('name', '')
            except Exception as _e:
                LOGGER.debug(f"Auto Rename IMDB lookup skipped: {_e}")
                _meta['imdb_name'] = _meta.get('name', '')
    ...
    # Generates a stylish IMDB-enriched caption for leech uploads.
    # Per-user toggle overrides global AI_CAPTION config:
    #   user sets True/False  → use that value
    #   user never set it     → fall back to global config_dict['AI_CAPTION']
    _ai_cap_user_val = user_dict.get('ai_caption', '')
    _ai_caption_enabled = (
        config_dict.get('AI_CAPTION', False)
        if _ai_cap_user_val == ''
        else bool(_ai_cap_user_val)
    )
    if not isMirror and dirpath and _ai_caption_enabled:
        try:
            from bot.helper.ext_utils.ai_caption import generate_ai_caption
            up_path = ospath.join(dirpath, prefile_)
            if await aiopath.exists(up_path):
                f_size = await aiopath.getsize(up_path)
                # Pass pre-parsed metadata so _rdx_parse_fields isn't called twice
                _meta = _rdx_parse_fields(prefile_)
                ai_cap = await generate_ai_caption(prefile_, f_size, dirpath, meta=_meta)
                if ai_cap:
                    cap_mono = ai_cap
                    LOGGER.info(f"AI Caption applied for: {prefile_}")
        except Exception as _ai_err:
            LOGGER.warning(f"AI Caption skipped for '{prefile_}': {_ai_err}")
    # ──────────────────────────────────────────────────────────────────────
