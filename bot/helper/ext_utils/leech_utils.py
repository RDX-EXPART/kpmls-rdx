...
async def get_tmdb_posters_list(file_name, limit=10):
    """
    Return list of poster candidates from TMDB.
    Each item: {title, year, url, source}
    Requires TMDB_API_KEY in config_dict.
    """
    api_key = config_dict.get("TMDB_API_KEY", "")
    if not api_key:
        LOGGER.warning("TMDB_API_KEY not set. Auto thumbnail poster skipped.")
        return []

    try:
        meta = _rdx_parse_fields(file_name)
        query = (meta.get("name") or ospath.splitext(file_name)[0] or "").strip()
        year = (meta.get("year") or "").strip()
        if not query:
            return []

        # Remove extra spaces for better search
        ...
        async with ClientSession() as session:
            async with session.get(url, timeout=20) as resp:
                if resp.status != 200:
                    LOGGER.warning(f"TMDB search failed for {query}: {resp.status}")
                    return []
        ...
        for item in data.get("results", []):
            ...
            pyear = date[:4] if date else year
            posters.append({
                "title": title,
                "year": pyear,
        ...
        LOGGER.warning(f"TMDB poster list error for {file_name}: {e}")
        return []


async def download_poster_thumb(poster_url):
    """Download poster URL and save as JPEG in Thumbnails. Returns local path or None."""
    if not poster_url:
        return None
    try:
        ...
        out_path = ospath.join(thumb_dir, f"poster_{int(time())}.jpg")

        async with ClientSession() as session:
            async with session.get(poster_url, timeout=25) as resp:
                if resp.status != 200:
                    LOGGER.warning(f"Poster download failed: {resp.status}")
                    return None
        ...
        LOGGER.warning(f"Poster thumb download error: {e}")
        return None


async def get_tmdb_poster_thumb(file_name):
    """Backward compatible: return first TMDB poster as local thumb path."""
    posters = await get_tmdb_posters_list(file_name, 1)
    if not posters:
        return None
    return await download_poster_thumb(posters[0].get("url"))
