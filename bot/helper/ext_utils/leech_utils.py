...
def _rdx_fix_series_title(title: str) -> str:
    ...
    Some sources use names like:
    Batchmates What the Engineering! S01 E01 ...
    where `What the Engineering!` is the episode title, not the series name.
    Keep the show name only when a clear episode-title phrase is detected.
    """
    if not title:
        return title
    cleaned = title.strip()
    ...
    # Very conservative episode-title heuristic. Only applies when the extra
    # part looks like a sentence/question/exclamation after a short show name.
    m = re_search(
