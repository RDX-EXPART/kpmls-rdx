...
def _rdx_fix_series_title(title: str) -> str:
    ...
    # Handles patterns like "Legends This Liverpool" → "Legends"
    # and "Show Name Episode Title" → "Show Name".
    # Only triggered for series (season+episode detected) with 3+ word titles.
    # SAFE: only trims when candidate is 1-2 words AND removed words are
    # all proper-noun-style (Title Case, not common connector words, no digits).
    ...
    words = cleaned.split()
    if len(words) >= 3:
        for trim_n in range(1, min(3, len(words))):
