...
def _rdx_parse_fields(raw_filename: str) -> dict:
    ...
    def _clean_source_tags(value: str) -> str:
        value = value or ""

        # Remove URLs, @channels, hashtags
        value = re_sub(r'https?://\S+|t\.me/\S+|www\.\S+', ' ', value, flags=IGNORECASE)
        value = re_sub(r'@\w+|#\w+', ' ', value, flags=IGNORECASE)

        # Remove generic website/source names:
        # HDHub4u, MoviesMod, Vegamovies, TamilMV, immo, frl, etc.
        value = re_sub(
            r'\b(?:www\s*)?\d*[a-z0-9]*'
            r'(?:movies?|movie|flix|hub|mod|mv|hd|web|dl|rip|ott|cinema|verse|world|zone|mart|wap|links?|drive|cloud|series|immo|frl|tamilmv)'
            r'[a-z0-9]*\b',
            ' ',
            value,
            flags=IGNORECASE
        ...
        # Remove leftover domain/source words
        value = re_sub(r'\b(com|net|org|in|cc|co|xyz|lol|ltd|is|tv|to|me|info|site|live|pro|dev|foo|www|frl|immo|rls|source|original|untouched)\b', ' ', value, flags=IGNORECASE)
        return value

    def _norm(value: str) -> str:
        value = _clean_source_tags(value)
        value = value.replace('_', ' ').replace('.', ' ').replace('-', ' ')
        value = re_sub(r'[\[\]\(\)\{\},+]', ' ', value)
        value = re_sub(r'\s+', ' ', value).strip()
        return value

    search_text = _norm(name_only)

    # Remove common release-time style junk like "6 00 P M", "10 30 PM"
    search_text = re_sub(r'\b\d{1,2}\s+\d{2}\s*(?:A\s*M|P\s*M|AM|PM)\b', ' ', search_text, flags=IGNORECASE)
    search_text = re_sub(r'\s+', ' ', search_text).strip()

    # Season / Episode:
    # S02E12, S02 E12, S02 EP12, S02 EP 12, S02 EP(10-12), Season 2 Episode 12
    season = episode = ''
    m = re_search(
        r'\b(?:S|Season)\s*(\d{1,2})\s*(?:E|EP|Episode)?\s*[\(\s]*(\d{1,3})(?:\s*(?:-|to|–)\s*(\d{1,3}))?',
        search_text,
        IGNORECASE
    )
    if m:
        season = f"S{int(m.group(1)):02d}"
        # If range exists, actual split episode file usually has its own E12 in filename.
        # Use detected episode from filename; if range found, use end episode only when no later episode exists.
        episode = f"E{int(m.group(2)):02d}"
        # Prefer last episode token if filename has another explicit E/EP later.
        all_eps = re.findall(r'\b(?:E|EP|Episode)\s*[\(]?\s*(\d{1,3})\b', search_text, flags=IGNORECASE)
        if all_eps:
            episode = f"E{int(all_eps[-1]):02d}"
    ...
    # Resolution
    rm = re_search(r'\b(144|240|360|480|540|576|720|1080|1440|2160|4320)p\b|\b(2K|4K)\b', search_text, IGNORECASE)
    resolution = f"{rm.group(1)}p" if rm and rm.group(1) else (rm.group(2).upper() if rm and rm.group(2) else '')

    # Quality
    quality = ''
    qualities = [
        'SDTV-Rip', 'HDTV-Rip', 'SDTVRip', 'HDTVRip', 'SDTV', 'HDTV', 'DVDScr',
        'PRE-HD', 'WEBDL', 'WEB-DL', 'WEB DL', 'WEBRip', 'BluRay', 'HDRip',
        'PreDVD', 'DVDRip', 'YTRip', 'HDTS', 'HDTC', 'CAMRip', 'HQSprint',
        'HDCAMRip', 'HDCAM', 'TELESYNC', 'TSQRip', 'BRRip', 'HQ SPrint', 'HQ HDRip', 'CAM', 'TRUE WEB-DL'
    ]
    for q in qualities:
        q_pattern = re_sub(r'[-\s]+', r'[- ]?', q)
        if re_search(rf'\b{q_pattern}\b', search_text, IGNORECASE):
            quality = 'WEB-DL' if q in ['WEBDL', 'WEB DL', 'TRUE WEB-DL'] else 'HDRip' if q == 'HQ HDRip' else q
            break
    ...
    ott_platforms = [
        ...
        'ZEE5', 'SS', 'APLVTV', 'HULU', 'APL', 'YT', 'STZ', 'STARZ', 'PS', 'AHA', 'HMAX', 'VOOT', 'TG'
    ]
    for o in ott_platforms:
        if re_search(rf'\b{re_sub(r"[-\s]+", r"[- ]?", o)}\b', search_text, IGNORECASE):
            ott = o
            break
    ...
    if lm:
        codec = lm.group(1).lower()
        lib = 'x265' if '265' in codec or codec == 'hevc' else 'x264' if '264' in codec or codec == 'avc' else codec.upper()

    # Audio
    audio = ''
    am = re_search(r'\b(DD\+?5\.1|DDP5\.1|DDP\d?(?:\.\d)?|AAC|AC3|OPUS|8CH|6CH|2CH|5\.1|7\.1|2\.0|ATMOS)\b', search_text, IGNORECASE)
    if am:
        audio = am.group(1).upper()

    # Subtitle short
    shortsub = ''
    sm = re_search(r'\b(?:HC-)?(?:[A-Za-z]{1,2}-)?([EeMmHh]Sub)\b', search_text, IGNORECASE)
    if sm:
        s = sm.group(1)
        shortsub = 'MSub' if s.lower().startswith('m') else 'HSub' if s.lower().startswith('h') else 'ESub'

    # Languages with common full names + short codes
    lang_map = {
        # Indian languages
    ...
    langs = []
    # Detect longer tokens first so Malayalam doesn't conflict with "mal"
    for k, v in sorted(lang_map.items(), key=lambda kv: len(kv[0]), reverse=True):
        if re_search(rf'\b{re_sub(r"[-\s]+", r"[- ]?", k)}\b', search_text, IGNORECASE) and v not in langs:
            langs.append(v)
    ...
    # Clean title only. GK-style: take everything before season/year/resolution,
    # then remove known junk. This protects movie titles better.
    title = search_text

    # Strong name boundary: title usually ends before season/year/resolution/quality.
    boundary = re_search(
        r'\b(?:S\s*\d{1,2}|Season\s*\d{1,2}|19\d{2}|20\d{2}|144p|240p|360p|480p|540p|576p|720p|1080p|1440p|2160p|4320p|4K|2K|WEB[- ]?DL|WEBDL|WEBRip|HDRip|BluRay|BRRip|DVDRip|HDTV|CAMRip)\b',
        title,
        IGNORECASE
    )
    if boundary:
    ...
    remove_patterns = [
        r'\b(19\d{2}|20\d{2})\b',
        r'\bS\s*\d{1,2}\s*(?:E|EP)?\s*\d{0,3}(?:\s*(?:-|to|–)\s*\d{1,3})?\b',
        r'\bSeason\s*\d{1,2}\b',
        r'\bEpisode\s*\d{1,3}\b',
        r'\b(144|240|360|480|540|576|720|1080|1440|2160|4320)p\b',
        r'\b(2K|4K)\b',
        r'\bWEB[- ]?DL\b', r'\bWEBDL\b', r'\bWEB DL\b', r'\bWEBRip\b',
        r'\bSDTV[- ]?Rip\b', r'\bHDTV[- ]?Rip\b', r'\bSDTVRip\b', r'\bHDTVRip\b', r'\bSDTV\b', r'\bHDTV\b',
        r'\bDVDScr\b', r'\bPRE[- ]?HD\b', r'\bHQ\b', r'\bHDRip\b', r'\bBluRay\b', r'\bBRRip\b',
        r'\bDVDRip\b', r'\bCAMRip\b', r'\bHQSprint\b', r'\bHQ SPrint\b', r'\bHDCAMRip\b', r'\bHDCAM\b',
        r'\bTELESYNC\b', r'\bTSQRip\b', r'\bYTRip\b', r'\bHDTS\b', r'\bHDTC\b', r'\bPreDVD\b', r'\bCAM\b',
        ...
        r'\b(DD\+?5\.1|DDP5\.1|DDP\d?(?:\.\d)?|AAC|AC3|OPUS|8CH|6CH|2CH|5\.1|7\.1|2\.0|ATMOS)\b',
        r'\b\d+(?:\.\d+)?\s*(GB|MB|KB)\b',
        r'\b\d+\s*Kbps\b',
        r'\b(?:HC-)?(?:[A-Za-z]{1,2}-)?[EeMmHh]Sub\b',
    ]
    ...
    title = _clean_source_tags(title)
    ...
    title = re_sub(r'\b\d{1,2}\s+\d{2}\s*(?:A\s*M|P\s*M|AM|PM)\b', ' ', title, flags=IGNORECASE)
