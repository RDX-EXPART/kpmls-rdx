...
def _rdx_parse_fields(raw_filename: str) -> dict:
    ...
    text = name_only.replace(".", " ").replace("_", " ").replace("-", " ")
    text = re_sub(r"\s+", " ", text).strip()

    # Season/Episode
    season = episode = ""
    m = re_search(r"\bS(\d{1,2})E(\d{1,3})\b", text, IGNORECASE)
    if m:
        season = f"S{int(m.group(1)):02d}"
        episode = f"E{int(m.group(2)):02d}"

    # Year / Resolution
    year_m = re_search(r"\b(19\d{2}|20\d{2})\b", text)
    year = year_m.group(1) if year_m else ""

    res = ""
    for r in _RDX_RES:
        if re_search(rf"\b{html_escape(r)}\b", text, IGNORECASE):
            res = r
            break
    ...
    quality = ""
    for q in _RDX_QUAL:
        if re_search(rf"\b{html_escape(q)}\b", text, IGNORECASE):
            quality = q
            break

    ott = ""
    for o in _RDX_OTT:
        if re_search(rf"\b{html_escape(o)}\b", text, IGNORECASE):
            ott = o
            break

    lib = ""
    for l in _RDX_LIB:
        if re_search(rf"\b{html_escape(l)}\b", text, IGNORECASE):
            lib = l
            break

    # Audio (basic)
    audio = ""
    am = re_search(r"\b(\d{1,2}CH|AAC|DDP\d?(?:\.\d)?|ATMOS|5\.1|7\.1|2\.0)\b", text, IGNORECASE)
    if am:
    ...
    shortsub = ""
    for s in _RDX_SUB:
        if re_search(rf"\b{html_escape(s)}\b", text, IGNORECASE):
            shortsub = s
            break

    # Languages
    langs = []
    for lg in _RDX_LANGS:
        if re_search(rf"\b{html_escape(lg)}\b", text, IGNORECASE):
            if lg.lower() in ("bengali", "bangla"):
                lg = "Bangla"
            langs.append(lg)
    # unique preserve
    seen = set()
    langs_u = []
    for lg in langs:
        if lg.lower() not in seen:
            langs_u.append(lg)
            seen.add(lg.lower())
    languages = ", ".join(langs_u)
    ...
    if len(langs_u) == 2:
        ...
        shortlang = f"Multi{len(langs_u)}"

    # Part (P01 etc)
    part = ""
    pm = re_search(r"\bP(\d{2})\b", text, IGNORECASE)
    if pm:
        part = f"P{pm.group(1)}"

    # Title only (remove tags)
    title = text
    title = re_sub(r"\b(19\d{2}|20\d{2})\b", " ", title)
    title = re_sub(r"\bS\d{1,2}E\d{1,3}\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(" + "|".join([html_escape(x) for x in _RDX_RES]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(" + "|".join([html_escape(x) for x in _RDX_QUAL]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(" + "|".join([html_escape(x) for x in _RDX_OTT]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(" + "|".join([html_escape(x) for x in _RDX_LIB]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(\d{1,2}CH|AAC|DDP\d?(?:\.\d)?|ATMOS|5\.1|7\.1|2\.0)\b", " ", title, flags=IGNORECASE)
    for lg in _RDX_LANGS:
        title = re_sub(rf"\b{html_escape(lg)}\b", " ", title, flags=IGNORECASE)
    for s in _RDX_SUB:
        title = re_sub(rf"\b{html_escape(s)}\b", " ", title, flags=IGNORECASE)
    ...
    return {
        ...
        "lib": lib,
        "languages": languages,
        "subtitles": "",      # from mediainfo placeholders already available separately
        "shortsub": shortsub,
        "shortlang": shortlang,
        "part": part,
    }
...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    # Extract extension early
    orig_ext = ('.' + prefile_.split('.')[-1]) if ('.' in prefile_ and len(prefile_.split('.')[-1]) <= 5) else ''
    raw_no_ext = prefile_[:-len(orig_ext)] if orig_ext and prefile_.endswith(orig_ext) else prefile_
