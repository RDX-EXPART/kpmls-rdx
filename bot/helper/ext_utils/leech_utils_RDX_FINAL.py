...
from aiofiles.os import re
from re import escape as re_escapemove as aioremove, path as aiopath, mkdir, makedirs, listdir
...
def _rdx_parse_fields(raw_filename: str) -> dict:
    ...
    res = ""
    for r in _RDX_RES:
        if re_search(rf"\b{re_escape(r)}\b", text, IGNORECASE):
            res = r
            break
    ...
    quality = ""
    for q in _RDX_QUAL:
        if re_search(rf"\b{re_escape(q)}\b", text, IGNORECASE):
            quality = q
            break

    ott = ""
    for o in _RDX_OTT:
        if re_search(rf"\b{re_escape(o)}\b", text, IGNORECASE):
            ott = o
            break

    lib = ""
    for l in _RDX_LIB:
        if re_search(rf"\b{re_escape(l)}\b", text, IGNORECASE):
            lib = l
            break
    ...
    shortsub = ""
    for s in _RDX_SUB:
        if re_search(rf"\b{re_escape(s)}\b", text, IGNORECASE):
            shortsub = s
            break
    ...
    for lg in _RDX_LANGS:
        if re_search(rf"\b{re_escape(lg)}\b", text, IGNORECASE):
    ...
    title = re_sub(r"\b(" + "|".join([re_escape(x) for x in _RDX_RES]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(" + "|".join([re_escape(x) for x in _RDX_QUAL]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(" + "|".join([re_escape(x) for x in _RDX_OTT]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(" + "|".join([re_escape(x) for x in _RDX_LIB]) + r")\b", " ", title, flags=IGNORECASE)
    title = re_sub(r"\b(\d{1,2}CH|AAC|DDP\d?(?:\.\d)?|ATMOS|5\.1|7\.1|2\.0)\b", " ", title, flags=IGNORECASE)
    for lg in _RDX_LANGS:
        title = re_sub(rf"\b{re_escape(lg)}\b", " ", title, flags=IGNORECASE)
    for s in _RDX_SUB:
        title = re_sub(rf"\b{re_escape(s)}\b", " ", title, flags=IGNORECASE)
