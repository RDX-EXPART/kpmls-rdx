...
#  Supported variables:
#    {title}        - Movie/series title (parsed name)
#    {year}         - Release year (e.g. 2025) or ""
#    {year_str}     - " (2025)" or "" if unknown
#    {year_line}    - Full year line or "" if unknown
...
#    {duration}     - File duration (e.g. 1h 35m) or ""
#    {duration_line}- Full duration line or "" if unknown
#    {subtitle}     - Subtitle info (e.g. ESub) or ""
#    {subtitle_line}- Full subtitle line or "" if unknown
#    {season}       - Season tag (e.g. S01) or ""
#    {episode}      - Episode tag (e.g. E05) or ""
#    {season_ep}    - Combined (e.g. S01E05) or ""
#    {season_ep_line}- Full episode line or ""
...
async def generate_ai_caption(
    ...
    try:
        ...
        # Subtitle from filename parser (fallback)
        subtitle_str = meta.get('shortsub', '') or meta.get('subtitles', '')

        # Duration — will be filled by ffprobe if available
        ...
        if dirpath:
            try:
                ...
                if await aiopath.exists(up_path):
                    ...
                    if subs:
                        # Build subtitle string: "ESub" or "MSub" or language list
                        sub_list = [s.strip() for s in subs.split(',') if s.strip()]
                        subtitle_str = 'MSub' if len(sub_list) > 1 else (sub_list[0] if sub_list else subtitle_str)
                    if dur:
        ...
        year_val = display_year or year or 'N/A'
        year_line = f"📅 <b>Year:</b> {year_val}\n" if (display_year or year) else ""

        imdb_line = f"🌟 <b>IMDB:</b> {imdb_rating}\n" if imdb_rating else ""

        # Duration
        duration_line = f"⏱ <b>Duration:</b> {duration_str}\n" if duration_str else ""

        # Subtitle
        subtitle_line = f"💬 <b>Subtitle:</b> {subtitle_str}\n" if subtitle_str else ""
