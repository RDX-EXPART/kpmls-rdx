...
# Cloudflare Worker — no API key or token needed
...
async def _tmdb_search(query: str):
    LOGGER.info(f"TMDB search: {query}")
    ...
    LOGGER.info(f"TMDB results: {len(results)}")
    if not results:
        return None

    if year:
        filtered = [x for x in results if (x.get("release_date") or x.get("first_air_date") or "")[:4] == year]
        if filtered:
            results = filtered
    ...
    if best:
        LOGGER.info(f"TMDB selected: {best.get('title') or best.get('name')} score={best_score}")
    return best


async def _tmdb_backdrop_urls(item, limit=5):
    """Fetch only landscape/backdrop images — portrait posters are excluded."""
    if not item:
        return []
    ...
    LOGGER.info(f"TMDB backdrops raw: {len(backdrops_raw)}")

    # Keep only entries that have a file_path — no strict size/aspect filter
    # so we match the same images as tmdb_helper.py
    valid = [img for img in backdrops_raw if img.get("file_path")]

    # Sort: English/no-language first, then highest vote_count
    en, oth, nul = [], [], []
    for img in valid:
    ...
    sorted_imgs = en or oth or nul

    urls = []
    for img in sorted_imgs[:limit]:
        fp = img.get("file_path")
        if fp:
            u = f"{TMDB_IMG}original{fp}"
            if u not in urls:
                urls.append(u)

    LOGGER.info(f"TMDB backdrops selected: {len(urls)}")
    return urls[:limit]
...
async def fetch_tmdb_poster_urls(name, limit=5):
    for query in _title_candidates(name):
        ...
        urls = await _tmdb_backdrop_urls(item, limit)
        if urls:
            LOGGER.info(f"TMDB backdrops found for: {query} ({len(urls)})")
...
async def _show_no_poster_menu(message, user_id, title):
    sent = await bot.send_message(
        ...
        text=(
            ...
            "No backdrop image was found. Tap Continue Upload."
        ),
...
async def choose_auto_thumb(message, file_name, user_id):
    ...
    sent = await bot.send_photo(
        ...
        photo=urls[0],
        caption=(
            "🎬 <b>Choose a backdrop for your upload.</b>\n\n"
            f"<b>Name:</b> <code>{escape(title)}</code>\n"
    ...
    if poster_path:
        return poster_path
    LOGGER.error("Selected backdrop could not be converted to a valid image; continuing without auto poster.")
    return None
...
async def auto_thumb_callback(client, query):
    ...
    if not state:
        return await query.answer("This menu expired.", show_alert=True)
    urls = state["urls"]
    if action == "noop":
        return await query.answer()
    if action == "skip":
    ...
    if action == "select":
        ...
        return await query.answer("Backdrop selected")
    if len(urls) <= 1:
        return await query.answer("Only one backdrop found", show_alert=True)
    if action == "next":
