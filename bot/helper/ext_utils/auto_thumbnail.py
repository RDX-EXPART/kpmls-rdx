...
def clean_movie_name(name: str) -> str:
    """Stable title cleaner used by both TMDB and old single-thumb API.
    Keeps the real title, removes quality/source/language/release junk.
    """
    raw = str(name or "")
    ...
    # Series: keep title before SxxExx / Season xx / Episode xx
    ...
    # Movies: if a year exists, keep title + year for better TMDB matching.
    ...
    # Remove everything after first known junk token.
    words = base.split()
    keep = []
...
def _title_candidates(name: str):
    ...
    # Try without year too, because TMDB sometimes matches better.
    add(sub(r"\b(19\d{2}|20\d{2})\b", "", title))

    # If title still contains junk, cut at common source words.
    add(sub(r"(?i)\b(Blu Ray|Bluray|WEB DL|WEBDL|WEBRip|Hindi|English|Bengali|Tamil|Telugu|ESub|x264|x265|HEVC).*", "", title))

    # Series packs often use: Series EpisodeTitle S02 E01 ...
    # Example: "Citadel Baked Alaskas S02 E01" should also try "Citadel".
...
async def _tmdb_request(path, params=None):
    ...
    return await request(f"{TMDB_API}{path}", params=params)
...
async def _tmdb_search(query: str):
    ...
    for item in results:
        ...
        if media_type not in ("movie", "tv") or item.get("adult"):
            continue
        ...
        score += float(item.get("vote_count") or 0) / 100
...
async def _tmdb_image_urls(item, limit=5):
    ...
    if media_type in ("movie", "tv") and tmdb_id:
        ...
        if isinstance(data, dict):
            images = data.get("backdrops") or []

        # Keep only real landscape backdrops and put the most useful ones first.
        # This reduces random dark scene screenshots and low-quality stills.
        filtered = []
        for img in images:
        ...
        # Prefer English/no-language, high votes, then high resolution.
        filtered.sort(
            key=lambda p: (
        ...
        for p in filtered[: max(limit * 2, limit)]:
            fp = p.get("file_path")
            if fp:
                urls.append(f"{TMDB_IMG}{fp}")
            if len(urls) >= limit:
                break
    clean = []
    for u in urls:
...
async def fetch_auto_thumb_urls(name, limit=5):
    # Primary: TMDB multiple poster carousel. Fallback: old single poster API.
...
async def _download_valid_poster(url):
    """Download any poster URL and convert/compress it to Telegram-safe JPG."""
    try:
        ...
        img.thumbnail((320, 320))  # Telegram thumbnail safe dimensions.
        quality = 85
        while True:
            bio = BytesIO()
