...
def _title_candidates(name: str):
    ...
    add(sub(r"(?i)\b(Blu Ray|Bluray|WEB DL|WEBDL|WEBRip|Hindi|English|Bengali|ESub|x264|x265|HEVC).*", "", title))
    return candidates[:4]
...
async def _tmdb_image_urls(item, limit=5):
    ...
    if media_type in ("movie", "tv") and tmdb_id:
        ...
        if isinstance(data, dict):
            posters = data.get("backdrops") or []
        # Prefer English/no-language posters, higher votes first.
        posters.sort(key=lambda p: (p.get("iso_639_1") not in ("en", None), -(p.get("vote_average") or 0), -(p.get("vote_count") or 0)))
        for p in posters:
            fp = p.get("file_path")
            if fp:
