...
def get_progress_bar_string(pct):
    ...
    rem    = (p / 100 * length) - filled

    # Animated Star Cluster — frame cycles on every message edit
    frame = int(time() * 2) % 4

    BODY = ['★', '✦', '★', '✧']   # filled cells shimmer
    TIP  = ['✨', '⭐', '✨', '🌟'] # leading tip sparkles
    EMPTY = '✧'
...
def get_readable_message():
    ...
    def get_progress_bar_dots(pct, length: int = 12):
        ...
        # Animated Star Cluster — changes on every status refresh
        frame = int(time() * 2) % 4

        BODY  = ['★', '✦', '★', '✧']   # filled cells cycle
        TIP   = ['✨', '⭐', '✨', '🌟'] # leading tip sparkles
    ...
    for i, download in enumerate(task_list, start=1):
        ...
        # ── Feature: Per-user Progress Bar + Emoji Pack (Auto Theme aware) ──────────
        try:
            from bot.modules.user_themes import get_user_progress_bar, get_user_emoji
            bar        = get_user_progress_bar(download.progress(), uid, length=12)
            _dl_emoji  = get_user_emoji(uid, 'download')
            _ul_emoji  = get_user_emoji(uid, 'upload')
            _seed_emoji= get_user_emoji(uid, 'seed')
            _wait_emoji= get_user_emoji(uid, 'wait')
            _spd_emoji = get_user_emoji(uid, 'speed')
            _size_emoji= get_user_emoji(uid, 'size')
            _time_emoji= get_user_emoji(uid, 'time')
        except Exception:
            # ── Auto Theme fallback ─────────────────────────────────
            try:
                ...
                _dl_emoji   = '📥'; _ul_emoji  = '📤'; _seed_emoji = '🌱'
                _wait_emoji = '⏸️'; _spd_emoji = '⚡'; _size_emoji = '📦'
                _time_emoji = '⏰'
        # ─────────────────────────────────────────────────────────────────────────
        ...
        msg += f"<b>┆ {bar}\n"
        msg += f"<b>┆ {_dl_emoji if 'Download' in status_text else _ul_emoji} Done » {download.processed_bytes()}</b>\n"
        msg += f"<b>┆ {_size_emoji} Total » {download.size()}</b>\n"
        msg += f"<b>┆ Status » {status_icon} {status_text}</b>\n"
        msg += f"<b>┆ {_spd_emoji} Speed » {download.speed()}</b>\n"
        msg += f"<b>┆ {_time_emoji} ETA » {download.eta()}</b>\n"
        ...
        try:
            ...
            if seeders is not None and leechers is not None:
                msg += f"┆ {_seed_emoji} Seeders »</b> {seeders} | <b>👥 Leechers »</b> {leechers}\n"
        except Exception:
            pass
    ...
    for download in download_dict.values():
        ...
        spd = download.speed()
        bps = convert_speed_to_bytes_per_second(spd)
...
async def get_content_type(url):
    ...
    Old code used a normal GET without timeout. Some hosters/CDNs delay that
    request for 3-10 seconds, so /mirror or /leech looked slow before the bot
    replied. HEAD is faster, and the fallback GET only requests the first byte.
    """
    timeout = ClientTimeout(total=2)
    headers = {"Range": "bytes=0-0", "User-Agent": "Mozilla/5.0"}
    try:
        async with aioClientSession(trust_env=True, timeout=timeout, headers=headers) as session:
            try:
                async with session.head(url, allow_redirects=True, verify_ssl=False) as response:
