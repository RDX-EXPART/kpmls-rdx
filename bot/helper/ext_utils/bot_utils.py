...
def get_progress_bar_dots(pct, length: int = 12):
    """Return a clean dot-based progress bar like: [●●●●●○○○○○○○] 41.67%"""
    try:
...
def get_readable_message():
    ...
    # ===== TASK LINES =====
    for i, download in enumerate(list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START], start=1):
        # File/Task name OUTSIDE the box
        name = escape(f"{download.name()}")
        if len(name) > 85:
            name = name[:82] + "..."
        ...
        # Dot progress bar like your reference
        ...
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
            ...
            if hasattr(download, 'seeders_num'):
                try:
                    msg += f"<b>│ Seeders 🌱 »</b> {download.seeders_num()}\n"
            ...
            msg += f"<b>│</b> {bar}\n"
            msg += f"<b>│ Status 🌱 »</b> <i>{download.status()}</i>\n"
            msg += f"<b>│ Size 📦 »</b> {download.size()}\n"
            msg += f"<b>│ Speed 🚀 »</b> {download.upload_speed()}\n"
            msg += f"<b>│ Uploaded 📤 »</b> {download.uploaded_bytes()}\n"
            msg += f"<b>│ Ratio 📊 »</b> {download.ratio()}\n"
            msg += f"<b>│ Time 🕒 »</b> {download.seeding_time()}\n"
            msg += f"<b>│ Engine 🛵 »</b> {download.eng()}\n"
        else:
            msg += f"<b>│ Status ⚙️ »</b> <i>{download.status()}</i>\n"
            msg += f"<b>│ Size 📦 »</b> {download.size()}\n"
            msg += f"<b>│ Engine 🛵 »</b> {download.eng()}\n"
    ...
    # ===== SPEED SUMMARY (kept from original logic) =====
    dl_speed = 0
    up_speed = 0
    ...
    # ===== SYSTEM MONITOR (clean, no duplicate) =====
    ...
    msg += "\n<b>⌬ SYSTEM MONITOR</b>\n"
    msg += f"⚡ CPU » {cpu}%  |  💾 FREE » {free_gb} [{free_p}%]\n"
    msg += f"🧠 RAM » {ram}%  |  🕒 UP » {up}\n"
    msg += f"📥 DL » {get_readable_file_size(dl_speed)}/s  |  📤 UL » {get_readable_file_size(up_speed)}/s\n"
