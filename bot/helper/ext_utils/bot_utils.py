...
def get_progress_bar_string(pct):
    ...
    # 8-level sub-character smooth fill: ▏▎▍▌▋▊▉█
    sub = ['▏', '▎', '▍', '▌', '▋', '▊', '▉', '█']
    bar = '█' * full
    if full < length:
        bar += sub[min(int(rem * 8), 7)]
        bar += '░' * (length - full - 1)

    # colour-coded tip: green → yellow → red
    if   p < 40:  tip = '🟢'
    elif p < 75:  tip = '🟡'
    else:         tip = '🔴'
...
def get_readable_message():
    ...
    def get_progress_bar_dots(pct, length: int = 12):
        ...
        # smooth partial-fill using 4-step quarter blocks
        partial_chars = ["▱", "▰", "▰", "▰"]
        tip_chars     = ["▷", "▶", "▶", "▶"]
