...
async def get_tmdb_posters_list(file_name, limit=10):
    """
    Fetch multiple poster results from TMDB.
    Returns a list of dicts: title, year, source, media_type, poster_url.
    """
    api_key = config_dict.get("TMDB_API_KEY", "")
    if not api_key:
        LOGGER.warning("TMDB_API_KEY not set. TMDB poster list skipped.")
        return []
    ...
    try:
        ...
        for item in data.get("results") or []:
            ...
            if media_type not in ("movie", "tv") or not poster_path:
                continue
            poster_url = f"https://image.tmdb.org/t/p/w500{poster_path}"
            if poster_url in seen:
                continue
            seen.add(poster_url)
...
async def download_poster_thumb(poster_url, prefix="poster"):
    """Download poster URL and save as local JPEG thumbnail. Returns path or None."""
    if not poster_url:
        return None
    try:
        ...
        out_path = ospath.join(thumb_dir, f"{prefix}_{int(time())}.jpg")

        async with ClientSession() as session:
            async with session.get(poster_url, timeout=20) as resp:
                if resp.status != 200:
        ...
        LOGGER.warning(f"Poster thumbnail download error: {e}")
        return None


async def get_tmdb_poster_thumb(file_name):
    """
    Backward-compatible helper: fetch first TMDB poster and save as local jpg.
    """
    posters = await get_tmdb_posters_list(file_name, limit=1)
    if not posters:
        return None
    return await download_poster_thumb(posters[0].get("poster_url"), "tmdb")
