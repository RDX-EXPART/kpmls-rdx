...
from bot.helper.ext_utils.leech_utils import get_audio_thumb, get_media_info, get_document_type, take_ss, get_ss, get_mediainfo_link, format_filename, get_tmdb_poster_thumb, get_tmdb_poster_thumb
    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            if (
                is_video
                and thumb is None
                and user_dict.get('auto_thumb', False)
                and user_dict.get('choose_poster', False)
            ):
