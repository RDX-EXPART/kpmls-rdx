...
from bot.helper.ext_utils.auto_thumbnail import get_auto_thumb_for_file
...
class TgUploader:
    ...
    def __init__(self, name=None, path=None, listener=None):
        ...
        self.__auto_thumbnail = False
        self.__choose_poster = False
    ...
    async def __user_settings(self):
        ...
        self.__auto_thumbnail = bool(user_dict.get('auto_thumbnail', False))
        self.__choose_poster = bool(user_dict.get('choose_poster', False) and self.__auto_thumbnail)
    ...
    async def __get_auto_thumb(self):
        if not self.__auto_thumbnail:
            return None
        # If Choose Poster is enabled, the same API poster is used as the selected poster.
        # This API currently returns one best poster/thumb for the title.
        try:
            thumb = await get_auto_thumb_for_file(self.__up_path)
            if thumb:
                LOGGER.info(f"[AUTO_THUMB] Applied auto thumbnail: {thumb}")
            return thumb
        except Exception as e:
            LOGGER.error(f"[AUTO_THUMB] Failed: {e}")
            return None
        ...
        try:
            ...
            if self.__as_doc or force_document or (not is_video and not is_audio and not is_image):
                ...
                if is_video and thumb is None:
                    thumb = await self.__get_auto_thumb()
                if is_video and thumb is None:
