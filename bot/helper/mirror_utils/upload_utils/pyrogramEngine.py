...
from bot.helper.ext_utils.leech_utils import get_audio_thumb, get_media_info, get_document_type, take_ss, get_ss, get_mediainfo_link, format_filename, get_tmdb_poster_thumb, download_poster_thumb
...
class TgUploader:
    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            user_dict = user_data.get(self.__user_id, {})
            selected_thumb_url = getattr(self.__listener, "selected_thumb_url", None)
            ...
            if is_video and thumb is None and user_dict.get('auto_thumb', False):
                if selected_thumb_url:
                    thumb = await download_poster_thumb(selected_thumb_url)
                elif thumb_choice == "tmdb":
