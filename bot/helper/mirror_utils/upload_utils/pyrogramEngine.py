...
from bot.helper.ext_utils.auto_thumbnail import choose_auto_thumb
...
class TgUploader:
    ...
    def __init__(self, name=None, path=None, listener=None):
        ...
        self.__auto_thumb_cache = {}
        # One auto-poster decision per upload task/folder.
        # For extracted series packs this prevents asking poster for every episode.
        self.__auto_thumb_task_checked = False
        self.__auto_thumb_task_value = None
    ...
    async def __buttons(self, up_path, is_video=False):
        ...
        # ── Streaming / Download buttons ──────────────────────────────────
        _base_url = config_dict.get('BASE_URL', '')
        if self.__stream_token and _base_url:
            try:
                from bot.helper.ext_utils.stream_link import build_stream_url
                _surl = build_stream_url(self.__stream_token, _base_url)
                buttons.ubutton('📥 Download', _surl + '?dl=1')
                buttons.ubutton('🌐 Stream', _surl)
            except Exception as e:
                LOGGER.error(f"Stream button error: {e}")
        # ─────────────────────────────────────────────────────────────────
        if config_dict['SAVE_MSG'] and (config_dict['LEECH_LOG_ID'] or not self.__listener.isPrivate):
            buttons.ibutton(BotTheme('SAVE_MSG'), 'save', 'footer')
        _has_stream = bool(self.__stream_token and _base_url)
        if self.__has_buttons or _has_stream:
    ...
    async def upload(self, o_files, m_size, size):
        ...
        for dirpath, _, files in sorted(await sync_to_async(walk, self.__path)):
            ...
            for file_ in natsorted(files):
                ...
                try:
                    ...
                    # Pre-generate stream token BEFORE upload so __buttons can embed the URL
                    _sbase = config_dict.get('BASE_URL', '')
                    if _sbase:
                        try:
                            from bot.helper.ext_utils.stream_link import create_stream_token
                            self.__stream_token = create_stream_token(file_)
                        except Exception as _se:
                            LOGGER.error(f"Stream token create error: {_se}")
                            self.__stream_token = None
                    else:
                        self.__stream_token = None
                    ...
                    if self.__is_cancelled:
                        return
                    # After upload: store file_id in the stream token entry
                    if self.__stream_token and self.__sent_msg and not self.__is_corrupted:
                        try:
                            from bot.helper.ext_utils.stream_link import set_stream_file_id
                            _s = self.__sent_msg
                            _fid, _mime = None, ''
                            if _s.video:
                                _fid, _mime = _s.video.file_id, 'video/mp4'
                            elif _s.document:
                                _fid = _s.document.file_id
                                _mime = _s.document.mime_type or 'application/octet-stream'
                            elif _s.audio:
                                _fid, _mime = _s.audio.file_id, 'audio/mpeg'
                            elif _s.animation:
                                _fid, _mime = _s.animation.file_id, 'video/mp4'
                            if _fid:
                                _msg_lnk = _s.link if (self.__listener.isSuperGroup or config_dict.get('LEECH_LOG_ID')) else ''
                                set_stream_file_id(self.__stream_token, _fid, _mime, _msg_lnk)
                        except Exception as _se:
                            LOGGER.error(f"Stream file_id update error: {_se}")
        ...
        try:
            if self.__auto_thumb_task_value and await aiopath.exists(self.__auto_thumb_task_value):
                await aioremove(self.__auto_thumb_task_value)
        except Exception as e:
            LOGGER.error(f"Auto thumbnail cleanup failed: {e}")
    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            # Auto Thumbnail API. Manual thumbnail has first priority.
            # Open the poster menu only once per upload task/folder.
            # If the user selects a poster, reuse it for all extracted episodes.
            # If the user skips/no poster, do not show the menu again for every episode.
            if not is_image and thumb is None and (is_video or is_audio):
                try:
                    if self.__auto_thumb_task_checked:
                        thumb = self.__auto_thumb_task_value
                        if thumb and not await aiopath.exists(thumb):
                            thumb = None
                    else:
                        self.__auto_thumb_task_checked = True
                        thumb = await choose_auto_thumb(self.__listener.message, file, self.__user_id)
                        self.__auto_thumb_task_value = thumb
                except Exception as e:
                    LOGGER.error(f"Auto thumbnail failed: {e}")
                    self.__auto_thumb_task_checked = True
                    self.__auto_thumb_task_value = None
                    thumb = None
            ...
            if self.__as_doc or force_document or (not is_video and not is_audio and not is_image):
                ...
                if thumb is not None:
                    try:
                        ...
                        LOGGER.error(f"Invalid thumbnail skipped: {thumb} | {e}")
                        try:
                            if await aiopath.exists(thumb):
                                await aioremove(thumb)
                        except Exception:
                            pass
                        self.__auto_thumb_cache[file] = None
                        thumb = None
                if thumb is None:
                    width = 480
                    height = 320
            ...
            # Do not delete the task-level auto selected poster after the first episode.
            # For extracted series packs, the same thumbnail must stay available for
            # every episode until the whole upload task is completed.
            if (
                self.__thumb is None
                and thumb is not None
                and thumb != self.__auto_thumb_task_value
            ...
            # Do not delete auto-selected thumbnail here. If send_video fails and
            # the uploader retries as document, the same thumbnail must still exist.
            # Cleanup is done only after a successful upload.
            LOGGER.error(f"{format_exc()}. Path: {self.__up_path}")
