...
class TgUploader:
    ...
    async def __prepare_upload_thumb(self, thumb):
        """Return a Telegram-safe JPEG thumbnail path or None.

        Telegram thumbnails must be real JPEG files, small, and <= 320x320.
        This also prevents selected auto posters from being ignored or causing
        retry uploads without thumbnails.
        """
        if not thumb or not await aiopath.exists(thumb):
            return None
        try:
            await makedirs("Thumbnails", exist_ok=True)
            out_path = ospath.join("Thumbnails", f"upload_thumb_{self.__user_id}_{int(time() * 1000)}.jpg")
            def _convert(src, dst):
                with Image.open(src) as img:
                    ...
                    while quality >= 45:
                        ...
                        if ospath.getsize(dst) <= 190 * 1024:
                            break
                        quality -= 10
                return dst
            await sync_to_async(_convert, thumb, out_path)
            if await aiopath.exists(out_path):
                return out_path
        except Exception as e:
            LOGGER.error(f"Thumbnail prepare failed: {thumb} | {e}")
        return None
    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            # Poster menu should open only once per file; retries reuse/skip cached result.
            if not is_image and thumb is None and (is_video or is_audio):
                try:
                    cache_key = self.__up_path
                    if cache_key in self.__auto_thumb_cache:
                        thumb = self.__auto_thumb_cache.get(cache_key)
                        if thumb and not await aiopath.exists(thumb):
                            thumb = None
                    else:
                        thumb = await choose_auto_thumb(self.__listener.message, file, self.__user_id)
                        self.__auto_thumb_cache[cache_key] = thumb
                except Exception as e:
                    LOGGER.error(f"Auto thumbnail failed: {e}")
                    self.__auto_thumb_cache[self.__up_path] = None
            ...
            # Always convert the selected/generated thumb to a Telegram-safe JPG
            # before uploading. Without this, Telegram may silently ignore it.
            prepared_thumb = None
            if not is_image and thumb is not None:
                prepared_thumb = await self.__prepare_upload_thumb(thumb)
                if prepared_thumb:
                    thumb = prepared_thumb
                else:
                    thumb = None
            ...
            if 'Telegram says: [400' in str(err) and key != 'documents':
                LOGGER.error(f"Retrying As Document with same thumbnail. Path: {self.__up_path}")
