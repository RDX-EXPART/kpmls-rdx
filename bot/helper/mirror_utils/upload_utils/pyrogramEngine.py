...
from bot.helper.ext_utils.auto_thumbnail import choose_auto_thumb, normalize_telegram_thumb
...
class TgUploader:
    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            # Make every selected/downloaded thumbnail Telegram-safe before upload.
            # This fixes uploads completing with no visible thumbnail.
            if not is_image and thumb is not None:
                safe_thumb = await normalize_telegram_thumb(thumb)
                if safe_thumb:
                    thumb = safe_thumb
                    self.__auto_thumb_cache[self.__up_path] = thumb
                else:
                    LOGGER.error(f"Invalid thumbnail skipped before upload: {thumb}")
                    self.__auto_thumb_cache[self.__up_path] = None
                    thumb = None
