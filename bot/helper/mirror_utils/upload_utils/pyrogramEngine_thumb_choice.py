    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            if is_video and thumb is None and user_dict.get('auto_thumb', False):
                if thumb_choice == "tmdb":
                    thumb = await get_tmdb_poster_thumb(file)
                elif thumb_choice == "gk":
                    # GK/API source will be added in leech_utils later.
                    # For now use TMDB as fallback so this button still works.
                    thumb = await get_tmdb_poster_thumb(file)
                elif thumb_choice == "ott":
                    # OTT source will be added in leech_utils later.
                    # For now use TMDB as fallback so this button still works.
                    thumb = await get_tmdb_poster_thumb(file)
