...
def gofile(url, auth):
    """
    Improved GoFile direct link generator (merged version)
    Supports folder and single file with guest token.
    """
    try:
        if "/d/" in url:
        ...
        # get guest token
        token_resp = session.get("https://api.gofile.io/accounts").json()
        token = token_resp["data"]["token"]

        api = f"https://api.gofile.io/contents/{file_id}?token={token}&cache=true"
        ...
        for content in children.values():
        ...
        # Get guest token (required by new GoFile API)
        token_resp = session.get("https://api.gofile.io/accounts").json()
        token = token_resp["data"]["token"]

        api = f"https://api.gofile.io/contents/{_id}?token={token}&cache=true"
        res = session.get(api).json()
