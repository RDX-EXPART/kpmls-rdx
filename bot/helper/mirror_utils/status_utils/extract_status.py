...
class ExtractStatus:
    ...
    def processed_raw(self):
        try:
            extract_path = getattr(self.__listener, 'extract_path', '')
            if extract_path:
                return max(0, async_to_sync(get_path_size, extract_path))
            if self.__listener.newDir:
                return max(0, async_to_sync(get_path_size, self.__listener.newDir))
            return max(0, async_to_sync(get_path_size, self.__listener.dir) - self.__size)
        except Exception:
            return 0
