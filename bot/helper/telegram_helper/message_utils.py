...
async def check_botpm(message, button=None):
    """Check whether user started bot PM, with short cache.

    Sending and deleting a PM on every /leech task adds extra Telegram API
    latency. Cache successful checks for 10 minutes to keep task response fast.
    """
    user_id = message.from_user.id
    cache_key = f"botpm_ok_{user_id}"
    if bot_cache.get(cache_key, 0) > time():
        return None, button

    try:
        temp_msg = await message._client.send_message(chat_id=user_id, text='<b>Checking Access...</b>')
        await deleteMessage(temp_msg)
        bot_cache[cache_key] = time() + 600
        return None, button
    except Exception as e:
