#!/usr/bin/env python3
"""
bot_commands.py — All bot commands including 4 new features:
  AutoThemeCommand   → /autotheme    (owner/sudo)
  BandwidthCommand   → /bandwidth    (owner/sudo)
  NightModeCommand   → /nightmode    (owner/sudo)
  MyThemeCommand     → /mytheme      (user)
  MyLevelCommand     → /mylevel      (user)
  SessionInfoCommand → /sessioninfo  (owner/sudo)
"""
...
class _BotCommands:
    def __init__(self):
        ...
        self.MyThemeCommand     = f'mytheme{CMD_SUFFIX}'
        self.MyLevelCommand     = f'mylevel{CMD_SUFFIX}'
        self.NightModeCommand   = f'nightmode{CMD_SUFFIX}'

        # ── NEW FEATURE COMMANDS ──────────────────────────────────────
        # Feature 1: Auto Dark/Light Theme
        self.AutoThemeCommand   = f'autotheme{CMD_SUFFIX}'

        # Feature 2: Smart Bandwidth Manager
        self.BandwidthCommand   = f'bandwidth{CMD_SUFFIX}'

        # Feature 4: Session Restore info
        self.SessionInfoCommand = f'sessioninfo{CMD_SUFFIX}'
