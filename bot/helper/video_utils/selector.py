...
class SelectMode():
    ...
    async def message_event_handler(self, mode=''):
        # Clear previous signal, then wait for the next input message.
        # Do not use Pyrogram's user() filter here; on some bot/userbot
        # combinations it can miss normal text messages. We filter safely
        # inside message_handler by chat id and user id instead.
        self.message_event.clear()
        pfunc = partial(message_handler, obj=self, is_sub=mode == 'subfile')
        handler = self.client.add_handler(MessageHandler(pfunc), group=-100)
        try:
