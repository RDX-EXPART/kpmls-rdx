...
class SelectMode():
    ...
    async def message_event_handler(self, mode=''):
        # Clear any previous signal before waiting for user input.
        # Without this, Rename can instantly finish and the next text will not be saved.
        self.message_event.clear()
        pfunc = partial(message_handler, obj=self, is_sub=mode == 'subfile')
        # Use an early handler group so normal bot command handlers do not swallow plain text input.
        handler = self.client.add_handler(MessageHandler(pfunc, user(self.user_id)), group=-1)
        try:
...
async def message_handler(_, message: Message, obj: SelectMode, is_sub=False):
    data = None
    # Accept input only from the same chat where the video-tool menu was opened.
    if message.chat.id != obj.message.chat.id:
        return

    if obj.is_rename and message.text:
        new_name = message.text.strip().replace('/', '').replace('\\', '')
        if not new_name or '.' not in new_name.rsplit(' ', 1)[-1]:
