...
from pyrogram.filters import regex, user, create
...
class SelectMode():
    ...
    async def message_event_handler(self, mode=''):
        # Wait for the next value message from the SAME user and SAME chat.
        # Rename and Trim both depend on normal text messages; Watermark/Subfile
        # depends on media. Using a real create() filter is more reliable than
        # adding a raw MessageHandler and filtering later, because other command
        # handlers/groups may consume or ignore messages differently.
        self.message_event.clear()
        chat_id = self.message.chat.id
        user_id = self.user_id
        need_subfile = mode == 'subfile'

        async def event_filter(_, __, event):
            user_ = event.from_user or event.sender_chat
            if not user_ or user_.id != user_id or event.chat.id != chat_id:
                return False

            if mode in ('rename', 'trim'):
                return bool(event.text)

            if mode in ('watermark', 'subfile'):
                if event.photo:
                    return not need_subfile
                if event.document:
                    return True
                if event.video or event.audio:
                    return False
                return bool(is_media(event))

            return bool(event.text or event.document or event.photo)

        pfunc = partial(message_handler, obj=self, is_sub=need_subfile)
        handler = self.client.add_handler(MessageHandler(pfunc, filters=create(event_filter)), group=-1)
        try:
