...
from asyncio import create_subprocess_exec, sleep, gather, Event, wait_for, TimeoutError
...
class VidEcxecutor:
    ...
    async def _send_status(self, status='wait'):
        # Keep the status card stable for video tools.
        # FfmpegStatus reads name/size from listener, so update them before
        # inserting the status object. Without this Video+Video can show an
        # empty filename and Total 0B.
        try:
            if self.name:
                self.listener.name = self.name
            elif self.path:
                self.listener.name = ospath.basename(self.path)
            if self.size:
                self.listener.size = self.size
            elif self.path and await aiopath.exists(self.path):
    ...
    async def run_ffmpeg(self, cmd: list, input_path: str, status: str) -> bool:
        ...
        try:
            await wait_for(progress_task, timeout=5)
        except TimeoutError:
    ...
    async def _merge_vids(self):
        ...
        for instruction in self.data['instructions']:
            ...
            # Update per-merge status fields before showing Video + Video.
            # This fixes missing filename, Total 0B and Done 0B on the status card.
            self.name = out_name or self.name or ospath.basename(group_files[0])
            self.listener.name = self.name
            try:
                self.size = 0
                for _vf in group_files:
                    self.size += await get_path_size(_vf)
            except Exception:
            ...
            for _vf in group_files:
                try:
                    total_duration += (await get_media_info(_vf))[0]
                except Exception:
                    pass
            self.listener.ffmpeg._total_time = total_duration
