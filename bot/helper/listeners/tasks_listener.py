...
# ── Feature: Session Restore + Inline Task Control ──────────────────
try:
    ...
    _SR_AVAILABLE = False
    async def sr_save_task(*a, **k): pass
    async def sr_remove_task(*a, **k): pass
    def register_retry(*a, **k): pass
    def unregister_retry(*a, **k): pass
# ────────────────────────────────────────────────────────────────────
...
class MirrorLeechListener:
    def __init__(self, message, compress=False, extract=False, isQbit=False, isLeech=False, tag=None, select=False, seed=False, sameDir=None, rcFlags=None, upPath=None, isClone=False, 
        ...
        self.suproc = None  # compatibility alias for old split helpers
        self.ffmpeg = None
        self.extract_path = ''
    ...
    def __parseSource(self):
        if self.source_url == self.message.link:
            ...
            if msg.startswith('magnet'):
                ...
                name = qs.get('dn', ['Unknown Magnet'])[0]
                hashh = qs.get('xt', ['urn:btih:Unknown'])[0].replace('urn:btih:', '')
                tracCount = len(qs.get('tr', []))
                self.source_msg = f"┎ <b>Name:</b> <i>{escape(name)}</i>\n┠ <b>Magnet Hash:</b> <code>{escape(hashh)}</code>\n┠ <b>Total Trackers:</b> {tracCount} \n┖ <b>Share:</b> <a href='https://t.me/share/url?url={quote(msg)}'>Share To Telegram</a>"
            else:
    ...
    async def onDownloadStart(self):
        ...
        # ── Session Restore: save task on start ──────────────────────
        task_type = 'leech' if self.isLeech else ('ytdlleech' if self.isYtdlp and self.isLeech else ('ytdl' if self.isYtdlp else ('clone' if self.isClone else 'mirror')))
        cmd_text  = self.message.text or ''
        await sr_save_task(
    ...
    async def onUploadComplete(self, link, size, files, folders, mime_type, name, rclonePath='', private=False):
        try:
            ...
            # ── Feature: Award XP for completed task ─────────────
            try:
                from bot.modules.user_levels import award_xp
                await award_xp(user_id, task_size_bytes=size if isinstance(size, int) else 0)
            except Exception:
                pass
            # ─────────────────────────────────────────────────────
            ...
            if self.isLeech:
                ...
                if not files:
                    ...
                    if config_dict['SAFE_MODE'] and self.isSuperGroup:
                        await sendMessage(self.message, message, buttons.build(2), photo=self.random_pic)
                    fmsg = '\n'
                        ...
                        if len(msg.encode() + fmsg.encode()) > (4000 if len(config_dict['IMAGES']) == 0 else 1000):
                            ...
                            if config_dict['SAFE_MODE']:
                                if self.isSuperGroup:
                                    ...
                                    await sendMessage(self.message, message + fmsg, buttons.build(2), photo=self.random_pic)
                            else:
                ...
                if config_dict['MIRROR_LOG_ID'] and not self.excep_chat:
                    ...
                    log_msg = list((await sendMultiMessage(config_dict['MIRROR_LOG_ID'], message, m_btns.build(2), self.random_pic)).values())[0]
                    if self.linkslogmsg:
                        dispTime = datetime.now(timezone(config_dict['TIMEZONE'])).strftime('%d/%m/%y, %I:%M:%S %p')
                        _btns = ButtonMaker()
                        if config_dict['SAVE_MSG']:
                ...
                if self.isPM:
                    if self.isSuperGroup:
                        ...
                        if self.botpmmsg:
                            await sendMessage(self.botpmmsg, message, buttons.build(2), photo=self.random_pic)
                            if config_dict['SAVE_MSG']:
                                s_btn.ibutton(BotTheme('SAVE_MSG'), 'save', 'footer')
                            s_btn.ibutton(BotTheme('CHECK_PM'), f"kpsmlx {user_id} botpm", 'header')
                            await sendMessage(self.message, message, s_btn.build(2), photo=self.random_pic)
                    else:
                        if self.source_url and config_dict['SOURCE_LINK']:
                            buttons.ubutton(BotTheme('SOURCE_URL'), self.source_url)
                        await sendMessage(self.message, message, buttons.build(2), photo=self.random_pic)
            ...
            if count == 0:
                ...
                await update_all_messages()
            # ── Session Restore: remove on upload complete ───────────
            await sr_remove_task(str(self.uid))
            unregister_retry(str(self.uid))
            # ─────────────────────────────────────────────────────────
            async with queue_dict_lock:
                if self.uid in non_queued_up:
                    non_queued_up.remove(self.uid)
    ...
    async def onDownloadError(self, error, button=None):
        ...
        self.sameDir = await remove_from_same_dir(self.uid, self.sameDir, self.folder_name)
...
┖ <b>Elapsed:</b> {get_readable_time(time() - self.message.date.timestamp())}'''
        ...
        if self.isSuperGroup and config_dict['INCOMPLETE_TASK_NOTIFIER'] and DATABASE_URL:
            await DbManger().rm_complete_task(self.message.link)
        # ── Session Restore: remove on error ────────────────────────
        await sr_remove_task(str(self.uid))
        unregister_retry(str(self.uid))
        # ─────────────────────────────────────────────────────────────
...
┖ <b>Elapsed:</b> {get_readable_time(time() - self.message.date.timestamp())}'''
        ...
        if self.isSuperGroup and config_dict['INCOMPLETE_TASK_NOTIFIER'] and DATABASE_URL:
            await DbManger().rm_complete_task(self.message.link)
        # ── Session Restore: remove on upload error ──────────────────
        await sr_remove_task(str(self.uid))
        unregister_retry(str(self.uid))
        # ─────────────────────────────────────────────────────────────
