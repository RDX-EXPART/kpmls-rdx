...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        # ===== RDX AUTO THUMBNAIL MENU START =====
        user_dict = user_data.get(self.user_id, {})
        if self.isLeech and user_dict.get('auto_thumb') and user_dict.get('choose_poster'):
            btn = ButtonMaker()
            btn.ibutton("TMDB 🎬", f"thumb {self.uid} tmdb")
            btn.ibutton("OTT 📺", f"thumb {self.uid} ott")
            btn.ibutton("GK API ⚡", f"thumb {self.uid} gk")
            btn.ibutton("Skip ❌", f"thumb {self.uid} skip")
            btn.ibutton("Done ✅", f"thumb {self.uid} done")

            self.thumb_choice = None
            self.thumb_event = Event()
            thumb_wait_dict[self.uid] = self

            self.thumb_menu_msg = await sendMessage(
                self.message,
                f"<b>🎬 Choose Thumbnail Source</b>\n\n<b>File:</b> <code>{escape(name)}</code>\n\n<i>Select a source, then press Done.</i>",
                btn.build_menu(2)
            )
...
async def auto_thumb_callback(client, query):
    ...
    if action == "skip":
        ...
        await query.answer("Auto thumbnail skipped.")

    elif action in ["tmdb", "ott", "gk"]:
        listener.thumb_choice = action
        await query.answer(f"Selected: {action.upper()}")

    elif action == "done":
        if listener.thumb_event:
            listener.thumb_event.set()
        return await query.answer("Thumbnail selection done ✅")
