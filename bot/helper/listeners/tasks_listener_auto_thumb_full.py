...
from bot.helper.ext_utils.leech_utils import split_file, format_filename, get_document_type, get_tmdb_posters_list
...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        # ===== RDX AUTO THUMBNAIL PREVIEW MENU START =====
        if self.isLeech and user_dict.get('auto_thumb') and user_dict.get('choose_poster'):
            try:
                self.thumb_posters = await get_tmdb_posters_list(name, 10)
                self.thumb_index = 0
                self.selected_thumb_url = None
                ...
                if self.thumb_posters:
                    ...
                    poster = self.thumb_posters[self.thumb_index]

                    btn = ButtonMaker()
                    btn.ibutton("⬅ Prev", f"thumbx {self.uid} prev")
                    btn.ibutton("Next ➡", f"thumbx {self.uid} next")
                    btn.ibutton("TMDB 🎬", f"thumbx {self.uid} tmdb")
                    btn.ibutton("OTT 📺", f"thumbx {self.uid} ott")
                    btn.ibutton("✅ Select This", f"thumbx {self.uid} select")
                    btn.ibutton("❌ Cancel", f"thumbx {self.uid} cancel")

                    caption = (
                        f"🖼 <b>{escape(poster.get('title', name))}</b>\n"
                        f"📅 <b>Year:</b> <code>{escape(str(poster.get('year', 'N/A')))}</code>\n"
                        f"🔗 <b>Source:</b> <code>{escape(poster.get('source', 'TMDB'))}</code>\n"
                        f"📌 <b>Poster:</b> <code>{self.thumb_index + 1}/{len(self.thumb_posters)}</code>\n\n"
                        f"<i>Choose a poster for your upload.</i>\n"
                        f"<b>Timeout:</b> 60s"
                    )

                    self.thumb_menu_msg = await bot.send_photo(
                        chat_id=self.message.chat.id,
                        photo=poster.get("url"),
                        caption=caption,
                        reply_to_message_id=self.message.id,
                        reply_markup=btn.build_menu(2)
                    )
                    ...
                    LOGGER.warning(f"No TMDB posters found for: {name}")
            except Exception as e:
                LOGGER.error(f"Auto Thumb Preview Error: {e}")
        # ===== RDX AUTO THUMBNAIL PREVIEW MENU END =====
...
async def auto_thumb_preview_callback(client, query):
    ...
    if action == "cancel":
        listener.selected_thumb_url = None
    ...
    if action == "select":
        if listener.thumb_posters:
            listener.selected_thumb_url = listener.thumb_posters[listener.thumb_index].get("url")
        if listener.thumb_event:
            listener.thumb_event.set()
        return await query.answer("Selected ✅")

    if action == "tmdb":
        listener.thumb_choice = "tmdb"
        return await query.answer("TMDB selected 🎬")

    if action == "ott":
        # OTT API will be added later; for now this keeps current TMDB results visible.
        listener.thumb_choice = "ott"
        return await query.answer("OTT selected 📺")

    if not listener.thumb_posters:
        return await query.answer("No poster found!", show_alert=True)

    if action == "next":
        listener.thumb_index = (listener.thumb_index + 1) % len(listener.thumb_posters)
    elif action == "prev":
        listener.thumb_index = (listener.thumb_index - 1) % len(listener.thumb_posters)
    else:
        return await query.answer("Unknown action!", show_alert=True)

    poster = listener.thumb_posters[listener.thumb_index]

    btn = ButtonMaker()
    btn.ibutton("⬅ Prev", f"thumbx {uid} prev")
    btn.ibutton("Next ➡", f"thumbx {uid} next")
    btn.ibutton("TMDB 🎬", f"thumbx {uid} tmdb")
    btn.ibutton("OTT 📺", f"thumbx {uid} ott")
    btn.ibutton("✅ Select This", f"thumbx {uid} select")
    btn.ibutton("❌ Cancel", f"thumbx {uid} cancel")

    caption = (
        f"🖼 <b>{escape(poster.get('title', 'Poster'))}</b>\n"
        f"📅 <b>Year:</b> <code>{escape(str(poster.get('year', 'N/A')))}</code>\n"
        f"🔗 <b>Source:</b> <code>{escape(poster.get('source', 'TMDB'))}</code>\n"
        f"📌 <b>Poster:</b> <code>{listener.thumb_index + 1}/{len(listener.thumb_posters)}</code>\n\n"
        f"<i>Choose a poster for your upload.</i>\n"
        f"<b>Timeout:</b> 60s"
    )
    ...
    try:
        await query.message.edit_media(
            InputMediaPhoto(media=poster.get("url"), caption=caption),
            reply_markup=btn.build_menu(2)
        )
    except Exception as e:
        LOGGER.error(f"Auto thumb preview edit error: {e}")
    await query.answer()


bot.add_handler(CallbackQueryHandler(auto_thumb_preview_callback, filters=regex(r"^thumbx ")))
